...
| **D6** | **P6 follow-on — the founder UI consolidation** | The actual page-merge (10 overview pages → The Letter) + deletion of the 32 dead redirect routes; lower `FOUNDER_ROUTE_BASELINE` as it shrinks. | A substantial client refactor needing the elite-multi-dimensional (viewport/pointer/theme/state) verification. The four-door *doctrine + ratchet* are already shipped. |
