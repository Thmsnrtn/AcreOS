...
Every shareable surface unfurls with the *same* static image: `OpenGraph.tsx` defaults `image` to `https://acreos.io/images/aerial_view_wide_hor_2dcf51b8.jpg` for the landing, the parcel-check, every /learn page, every county page, the calculator. When a land investor pastes a `/learn/land-flipping/texas` link into a BiggerPockets DM or an X reply, the card should say "Land flipping in Texas — §5.077, executory contracts, the close" over a branded card with the AcreOS mark — not a generic aerial photo that looks like every other land site. The unfurl IS the ad in organic distribution, and ours is undifferentiated.
