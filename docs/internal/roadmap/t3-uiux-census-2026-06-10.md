...
| # | Item | Files | Effort |
|---|---|---|---|
| W3-1 | ✅ SHIPPED — retire/refit split executed 2026-06-11. RETIRED (pages deleted, routes → /founder/bridge redirect): sovereign-dashboard, sovereign-v13, anticipatory-enterprise, agent-collaboration. REFIT to /founder/* + elevated to house standard (shaped Skeletons role="status"+sr-only+announce={false}, QueryErrorState w/ retry, EmptyState/EmptyFilter, aria-labels, lib/format dates): board-of-directors→/founder/governance, memory-browser→/founder/memory, event-log→/founder/event-log, conscious-organization→/founder/scenarios — old paths redirect. use-sovereign-dashboard.ts queryFns now throw on !res.ok (were silently returning []/null). Sidebar/nav-items/production-smoke/sim-founder-journey repointed. agent-performance + job-health kept as-is (already founder-core routes; agent-performance was B-grade). Server /api/founder/v13/* + sovereign endpoints untouched (shared) | 14 files moved/deleted + App.tsx/nav/2 specs | M (retire) / L (refit) |
