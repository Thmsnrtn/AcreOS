...
16. **No way to embed AcreOS inside Kajabi.** Kajabi's "Custom Code" block accepts arbitrary HTML/JS. I want an `<iframe src="acreos.com/embed/{tenant}/today" data-jwt="...">` that lives inside lesson 4 so students never leave the course platform. Today the auth model assumes full-page browser context. Need a postMessage-based iframe SDK + Clerk session forwarding.
