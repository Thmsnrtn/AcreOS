...
### Option 1: Release the distributed lock (for single job)
```bash
# The job lock is stored in the job_locks table
# Find and delete the lock for the stuck job:
# psql $DATABASE_URL -c "
# DELETE FROM job_locks WHERE job_name = 'deal_hunter_scraping' AND locked_until < NOW() + INTERVAL '1 hour';
# "
```

### Option 2: Graceful restart of one instance
```bash
...
### Option 3: Emergency stop (if Option 1/2 insufficient)
```bash
# Kill specific machine
fly machine stop <machine-id> -a acreos

# Fly.io will start a replacement automatically (min_machines_running = 2)
```
...
- Every external HTTP call must have a timeout
