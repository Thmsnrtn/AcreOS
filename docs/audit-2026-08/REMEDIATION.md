...
| **P2** | Campaign SMS fell back to the **platform** Twilio account when the org had no BYO identity — a "be the rail" constitution breach; the fix comment even claimed the opposite | hardened at the CHOKEPOINT: `sendOrgSMS` (the counterparty path) now sets `requireByoIdentity`, so `twilioProvider.sendSms` resolves BYO-only and never falls back to the platform account — checked per-send, closing the mid-batch TOCTOU an adversarial re-verify of the first fix flagged; plus the campaign-level pre-loop gate (`orgHasConnectedSmsIdentity`) for fast-fail UX + full refund, and the false comment corrected |
