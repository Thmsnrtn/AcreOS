...
|---|---|---|---|
| **Founder narrative routers V6–V14** (sovereign-company, learning-company, living-organization, conscious-organization, anticipatory-enterprise, real-runtime, sentient-enterprise, self-running-company) | **~2,763 LOC in 8 RENAMED routers** — V6=`routes-founder-sovereign-company.ts`, V7=`-learning-company`, V8=`-living-organization`, V10=`-conscious-organization`, V11=`-anticipatory-enterprise`, V12=`-real-runtime`, V13=`-sentient-enterprise`, V14=`-self-running-company` — mounted `routes.ts:1865-1879` via `registerFounderV6..V14Routes`, **plus 43 `*V[6-14].ts` service suites (~17,328 LOC)** still imported by them. | **KILL — NOT YET EXECUTED (verified 2026-08-06 audit)** | Dead-mounted narrative theater: the founder UI doors were retired (`nav-items.ts`), no client pages consume these APIs. **⚠ STALE-POINTER CORRECTION (2026-08-06):** the routers were RENAMED from the old `routes-founder-v6.ts…v14.ts` names (which no longer exist) to the narrative names above, and the mount moved to `routes.ts:1865-1879` (was cited `:1792-1807`). A fresh session that greps the old filenames finds nothing and wrongly concludes this KILL was executed — it was NOT. The unverified "several synthetic (`Math.random`)" claim is dropped (sampled routers/services show none). Keep only the two refit pages that survived (`/founder/scenarios`, `/founder/governance`) and their real backing services; delete the rest — still the campaign's single biggest LOC win (~20K). |
