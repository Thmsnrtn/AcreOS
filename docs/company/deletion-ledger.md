...
| **V12/V13 agent-infrastructure dead methods** (the org-scope register's UNREACHABLE cluster) | `cognitiveMemoryV13.ts` (recordEpisode, recallEpisodes, getWorkingMemory, setWorkingMemory, clearExpiredWorkingMemory, consolidateMemories, getMemoryStats), `selfHealingMeshV13.ts` (detectAnomalies, performRootCauseAnalysis, evaluatePlaybooks, resolveAnomaly, getHealthDashboard, getStats), `adaptiveStrategyV13.ts` (deactivateStrategy, getStrategyLineage, getStats), `governanceBrainV13.ts` (explainAction, getExplanation, getStats), `eventMeshV12.ts` (replayEvents, getEventsByType), `ceoReminders.ts` (getContextForReminder) | **KILL-pending — founder ruling required (found 2026-09-05)** | **Zero call sites, verified adversarially.** These are the 30 entries the tenancy register (`scripts/org-scope-route-widening.json`) had been holding under the placeholder verdict UNREACHABLE: the flag is real — each reads an org-tagged table with no organization predicate — but the method has no production caller, so nothing is live. Read by a fan-out of one agent per file, then every claim put to an independent agent briefed to REFUTE it; 24 of 30 dispositions survived unchanged, 6 were corrected, and the corrections are the reason this is a ledger row rather than a deletion commit. **Why not deleted here:** this ledger's own rule, written on the `EnvironmentalIntelligenceCard` row — *"do not delete opportunistically … deleting it here would be exactly the opportunistic deletion the execution rules forbid"*. These were found while burning down a tenancy register, not under a deletion ruling. **Two corrections that must not be lost, because a later ruling would otherwise rest on a false premise.** (1) **`agent_semantic_memory.org_id` IS populated by a live path** — `routes-founder-self-running-company.ts` → `feedbackLoopV14.propagateLearning` → `cognitiveMemoryService.extractFact({ … orgId: learning.orgId })`, where `feedback_learnings.orgId` is `.notNull()`. The first-pass note on several of these entries said the lane was never populated and that scoping would therefore filter every row to nothing; that is FALSE for the semantic table and must not be carried into the OD-8 drop evidence. Any drop decision on these tables needs live `count(*)` / `count(*) filter (where org_id is not null)` figures, not a repo grep. (2) **`collaborationProtocolV13`'s dialogue lane is NOT in this row** — its methods are equally caller-less, but `agent_dialogues` has a LIVE consumer: `autonomyFinalMile.executeResolvedConsensus`, rostered as `execute_resolved_consensus` and running every five minutes, reads rows with `status = 'consensus_reached'` and hands `resolution.action` / `resolution.input` to `executionEngine.execute` under the row's own `orgId`. Its producer route was retired 2026-08-29, so it can only replay decisions reached before then. The mechanism is bounded — `validateSafetyGates` runs first, an unregistered action is REFUSED as `no_executor_registered` rather than reported done, and each row flips to `closed` after execution so the backlog drains once rather than replaying — but *what those rows are* is a production question this session cannot answer without querying the live database. Count them before ruling on that lane. **Ratchet collateral, so the ruling is cheap to execute** (each must land in the SAME commit as its deletion or CI fails): removing `governanceBrainV13`'s two explanation methods leaves `audit_explanations` with neither writer nor reader — `tables-no-writer` 57→58 and `tables-no-reader` 67→68, both strictly down-only, so it needs three `scripts/ratchets/reachability.json` allowlist entries in the `pax_drafts` shape ("AWAITING THE OD-8 DROP BATCH") or the table taken in that batch instead; `adaptiveStrategyV13.getStats` is the last reader of `strategy_assignments` and `strategy_proposals` (+2 more); `ceoReminders.getContextForReminder` lowers `baselines.unreachedExports` 370→369 (366 if `markFired`/`snoozeReminder`/`processRecurringReminders` go with it); `recallEpisodes`+`recordEpisode` must go together — deleting only the reader leaves the one function that can repopulate the tenant lane — and `tests/unit/cascadePrecedentHit.test.ts`'s factory mock of both must be deleted in the same commit, since a factory mock is not validated against the real module and would silently mock methods that no longer exist. Every register key removed must take its `_TRIAGED` reason with it (`orgScopedFetchCoverage.test.ts` fails on an orphan, `check-org-scoped-fetch.mjs` fails on a stale key). **If the ruling is instead to KEEP any of them, the fix is `scope`, not `leave`** — the precedent is `negotiationOrchestrator.runNegotiationAssistant`: an unreachable function with this shape is what someone wires up later believing it safe. |
| **`matchmaking.calculateSimilarity`** | `server/services/matchmaking.ts:344-395` (module-private; only `export const matchmaking` escapes) | **KILL-pending — founder ruling required (found 2026-09-05)** | **Zero call sites**, confirmed by an adversarial second read that could not break the census (case-insensitive name search, dynamic dispatch, string-keyed lookup). It is also a latent cross-tenant read: it compares profiles with no organization predicate. It is NOT deleted and NOT scoped here for one reason — `matchmaking.ts` sits under the **Marketplace FREEZE** row above, and touching a frozen module on an agent's own judgement is the thing that row exists to prevent; the `scoreCountyForTargeting` row is the precedent for recording rather than acting. **Salvage note, so a ruling is cheap:** the four live marketplace flows route through `calculateMatchScore` (`matchmaking.ts:122`) and never through `calculateSimilarity`, so reactivation at G2 does not depend on it. **Hard precondition if it is ever wired:** add the organization predicate or delete the method BEFORE any caller exists. **If the ruling is KILL,** two edits must land in ONE commit with the method body: the rule-1 key in `scripts/org-scope-route-widening.json` AND its `_TRIAGED` entry — the key alone goes stale and fails `check-org-scoped-fetch.mjs`, the reason alone becomes an orphan and fails `orgScopedFetchCoverage.test.ts`. Verified unaffected either way: `marketplaceAuthority.test.ts` and `marketplaceLadderIsClosed.test.ts` both assert on `findBuyersForListing`, far from this method. |
| **Trust ledger (single-entry) runtime** | `GET/POST /api/trust-ledger`, `GET /api/trust-ledger/balance`, `GET /api/notes/:noteId/trust-ledger` (`routes-va-engine.ts`); `getTrustLedgerEntries` / `getTrustLedgerByNote` / `createTrustLedgerEntry` / `getTrustBalance` (`closingServicingRepo.ts`) + their `IStorage` declarations; `recordLedgerEntry` / `recordNotePayment` / `recordDealAcquisition` / `recordDealSale` / `generateProfitLoss` and the `LedgerEntryType` / `LedgerJournalEntry` / `ProfitLossStatement` types (`services/bookkeeping.ts`); an unused `trustLedger` import in `commissionService.ts`. Table `trust_ledger`. | **KILL — RUNTIME EXECUTED 2026-09-06; TABLE LEFT INERT** | **Zero callers, verified adversarially and then re-verified at HEAD.** No client page, no AI tool dispatcher, no other service reached any of the four routes; the four repo methods were reached only by those routes; the five bookkeeping journal functions had zero call sites of any kind. **It was not merely unused, it was wrong.** `getTrustBalance` returned the stored `running_balance` of the newest row — a number the server never computed — while `POST /api/trust-ledger` spread `req.body` wholesale, so an operator could set their own trust balance to any value and the entry list would not sum to the balance the API reported. On a surface named for a fiduciary trust account. And the journal block's own header called it "DOUBLE-ENTRY JOURNAL ENTRIES" when `recordLedgerEntry` wrote ONE row carrying a denormalised running total. **Why retired rather than hardened:** the org's real books are `account_ledger_entries` (`shared/schema/accounting-ops.ts`) — genuinely double-entry, CHECK-constrained, and actually read by `trialBalance`, `glPdfExport`, `qboExport` and `recognitionWorker`. Two ledgers was one too many and the wrong one was the one nothing used; making unreachable machinery honest buys nothing and leaving it invited someone to wire it up believing it sound. **Premise correction vs. DEFECT-0053:** the registry framed this as fiduciary/regulatory risk from a last-row running balance. At HEAD the endpoints had no callers, so no screen ever showed the number; the real exposure was a direct API consumer being told an authoritative-sounding balance the server never derived. Recorded as what it was. **Ratchet collateral, landed in the same commit:** `internalOnlyExports` 1169→1168 and `opaqueExports` 20→16 (down moves from the deletion); `tables-no-writer` and `tables-no-reader` are NOT bumped — `trust_ledger` is allowlisted in the `pax_drafts` shape instead, because raising those baselines would record a table nothing can write as an accepted floor rather than as a drop awaiting authority. **FOUNDER DECISION OUTSTANDING — the table only.** `trust_ledger` is untouched and inert. Dropping it may delete customer financial rows, which is a founder-only hard stop. Evidence needed before the ruling is a live `SELECT count(*), count(*) FILTER (WHERE running_balance IS NOT NULL), min(created_at), max(created_at) FROM trust_ledger;` — this session has no production DB access and will not guess at it. Recommendation: if the count is zero, drop it in the next migration batch; if non-zero, export the rows to the org's accounting record first, then drop — the data cannot be reconstructed from `account_ledger_entries` because nothing ever wrote both. |
