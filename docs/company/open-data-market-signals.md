...
**Discovery-pipeline status (2026-07-28 run — live point-intersection attempts from `server/services/openData/discoveryPipeline.ts`; evidence pinned in `PIPELINE_RUN_2026_07_28`):**

- **NY — PROMOTED 2026-07-28.** Live-verified: `gisservices.its.ny.gov/arcgis/rest/services/NYS_Tax_Parcels_Public/MapServer` layer 1, point query at the NYS Capitol (42.6529, −73.7594) returned `PRINT_KEY=76.10-1-10.10`, `PRIMARY_OWNER`, `TOTAL_AV`, `FULL_MARKET_VAL`. License: this doc already lists the program as verified free. Seeded as a statewide row; only ~38 opted-in counties — elsewhere the layer returns zero features and lookups fall through (TN model).
- **WI — PROMOTED 2026-07-28.** Live-verified: `services3.arcgis.com/n6uYoouQZW75n5WI/.../Wisconsin_Statewide_Parcels_DB/FeatureServer` layer 0 (V12, WI DOA/WLIP/SCO), point query at the WI Capitol (43.0747, −89.3844) returned `PARCELID=070924205010`, `OWNERNME1`, `CNTASSDVALUE`. License: item terms state verbatim "This data free for public consumption as of: 06/30/2026" — open. Seeded.
- **MD — PROMOTED 2026-07-28.** Live-verified: `mdgeodata.md.gov/imap/rest/services/PlanningCadastre/MD_ParcelBoundaries/MapServer` layer 0, point query at the MD State House (38.9788, −76.4910) returned `ACCTID=020600007679310`, `NFMTTLVL`, `ACRES`. License: official MD iMAP item terms state "The Data can be freely distributed as long as the metadata entry is not modified or deleted" — open with attribution/metadata condition. Seeded. Caveat: no owner-NAME field in the layer (owner mailing address only).
- **VT — verified, NOT seeded (license unresolved) 2026-07-28.** Live-verified: VCGI standardized parcels FeatureServer (point in Montpelier returned `SPAN=405-126-13891`, `OWNER1`, `ACRESGL`, `REAL_FLV`), but the published item terms found this pass are warranty disclaimers only — no affirmative open/free statement located. Stays unseeded pending a real license determination.
- **UT — verified, NOT seeded (license unresolved) 2026-07-28.** Live-verified: UGRC `UtahStatewideParcels` FeatureServer (point at the UT Capitol returned `PARCEL_ID=09311610020000`; no owner-name field), but item terms are an as-is warranty disclaimer only — no affirmative open/free statement located this pass. Stays unseeded.
- **MA — verified, NOT seeded (license unresolved) 2026-07-28.** Live-verified: MassGIS `Massachusetts_Property_Tax_Parcels` FeatureServer (point at the MA State House returned `PROP_ID=0501615000`, `OWNER1`, `TOTAL_VAL`), but item terms found this pass are accuracy disclaimers only — no affirmative open/free statement located. Stays unseeded.
- **OR — verify FAILED 2026-07-28.** No public statewide REST query service found: the DOR `DOR_ORMAP_TAXLOTS` service is secured (HTTP 403 via utility.arcgis.com); AGOL search surfaced county-level taxlot services only. Consistent with "coverage still maturing" above. Stays a candidate.
- **KS — EXCLUDED (license) 2026-07-28.** Bulk/service terms remain UNCLEAR per this doc, so KS is hard-excluded in the pipeline regardless of technical reachability. This run also found no statewide parcel query layer on `services.kansasgis.org` (ORKA folder exposes address points/subdivisions/lots only).
