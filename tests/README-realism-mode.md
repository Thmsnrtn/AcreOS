...
| Layer B (full logged-in walk) | ⛔ **Blocked by a hard architectural constraint (diagnosed).** Staging uses `pk_live` → frontend host `clerk.acreos.io` = the **prod Clerk instance, origin-locked to acreos.io**. A browser session cannot be established on `acreos-staging.fly.dev` (wrong origin; cookies scoped to acreos.io). Proven: minting a real Clerk user + sign-in token via the backend API **works** (backend isn't origin-scoped), and the auth gate **works** (401), but redeeming the ticket in a real browser bounces back to `/auth`. **Fix is a Clerk-dashboard change only the founder can make:** either (A) a dedicated Clerk **test instance** with `acreos-staging.fly.dev` in allowed origins (clean), or add the staging domain as an allowed origin / satellite on the existing instance. Then `realism-audit.yml`'s Layer B (`auth-clerk-ticket.setup.ts` + `CLERK_SIGN_IN_TICKET`) runs in CI, where `CLERK_SECRET_KEY` lives as a GitHub secret (never on a dev machine). |
