...
describe("createRateLimiter", () => {
  ...
  describe("allows requests below threshold", () => {
    it("calls next() for requests within the limit", async () => {
    ...
    it("decrements X-RateLimit-Remaining with each request", async () => {
      ...
      await limiter(req, res, next); // remaining = 2
      expect(headers["X-RateLimit-Remaining"]).toBe("2");

      await limiter(req, res, next); // remaining = 1
      expect(headers["X-RateLimit-Remaining"]).toBe("1");

      await limiter(req, res, next); // remaining = 0
      expect(headers["X-RateLimit-Remaining"]).toBe("0");

      expect(next).toHaveBeenCalledTimes(3);
    });

    it("allows a single request without blocking", async () => {
  ...
  describe("returns 429 after exceeding threshold", () => {
    it("blocks the request that exceeds the limit", async () => {
      ...
      await limiter(req, res, next); // 1 — allowed
      await limiter(req, res, next); // 2 — allowed
      await limiter(req, res, next); // 3 — blocked

      expect(next).toHaveBeenCalledTimes(2);
      expect(res.status).toHaveBeenCalledWith(429);
    });

    it("returns JSON body with message and retryAfter fields on 429", async () => {
      const limiter = createRateLimiter({ maxRequests: 1, windowMs: 60_000 });
      const { req, res, next } = mockReqRes("10.0.1.2");

      await limiter(req, res, next); // allowed
      await limiter(req, res, next); // blocked
    ...
    it("sets X-RateLimit-Remaining to 0 on a 429 response", async () => {
      const limiter = createRateLimiter({ maxRequests: 1, windowMs: 60_000 });
    ...
    it("continues blocking after the first 429", async () => {
      const limiter = createRateLimiter({ maxRequests: 1, windowMs: 60_000 });
      const { req, res, next } = mockReqRes("10.0.1.4");

      await limiter(req, res, next); // allowed
      await limiter(req, res, next); // blocked #1
      await limiter(req, res, next); // blocked #2

      expect(next).toHaveBeenCalledTimes(1);
      expect(res.status).toHaveBeenCalledTimes(2);
    });

    it("isolates limits per IP — different IPs are independent", async () => {
      const limiter = createRateLimiter({ maxRequests: 1, windowMs: 60_000 });
      const a = mockReqRes("10.0.2.1");
      const b = mockReqRes("10.0.2.2");

      await limiter(a.req, a.res, a.next); // IP A — allowed
      await limiter(a.req, a.res, a.next); // IP A — blocked

      await limiter(b.req, b.res, b.next); // IP B — allowed (fresh limit)
  ...
  describe("returns Retry-After header on 429", () => {
    ...
    it("Retry-After matches retryAfter in the JSON body", async () => {
      const limiter = createRateLimiter({ maxRequests: 1, windowMs: 60_000 });
  ...
  describe("custom key function", () => {
    it("uses the provided key function instead of IP", async () => {
  ...
  describe("clearRateLimitStore", () => {
    it("resets the store so previously blocked IPs can proceed", async () => {
      const limiter = createRateLimiter({ maxRequests: 1, windowMs: 60_000 });
      ...
      await limiter(req2, res2, next2); // should be allowed again
