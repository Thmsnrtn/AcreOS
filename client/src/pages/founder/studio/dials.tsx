/**
 * /founder/studio → "Dials" tab — the original generic renderer over
 * founder_settings. Extracted from studio.tsx unchanged so the new
 * sub-surfaces (allocation, credits, triggers, routing, byok, infra) can
 * coexist as peer tabs without touching the legacy dial UX.
 */
...
export default function FounderStudioDials() {
  ...
  if (type === "enum" && dial.validRange?.oneOf) {
    return (
      <Select value={String(value)} onValueChange={(v) => onChange(v)}>
        <SelectTrigger className="font-mono" aria-label={dial.key}>
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
