    ...
    <PageShell label="Prompt History">
      <div className="space-y-6 max-w-5xl mx-auto">
        ...
        {historyLoading ? (
          ...
          <EmptyState
            icon={GitBranch}
            headline="No agents with history yet"
            subtitle="Once the meta-agent ships its first prompt change, versions will start accumulating here."
            // TODO(cta): prompt history is system-generated by the meta-agent cron
            cta={{ label: "", _noOp: true }}
          />
        ) : versions.length === 0 ? (
          <EmptyState
            icon={History}
            headline={`No versions recorded for ${selectedAgent}`}
            subtitle="Either this agent hasn't had a prompt change yet, or its history predates version control."
            // TODO(cta): version history is system-generated; no direct user action creates versions
            cta={{ label: "", _noOp: true }}
          />
        ) : (
