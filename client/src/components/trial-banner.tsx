...
export function TrialBanner() {
  ...
  // Show "Start Free Trial" prompt for eligible users
  if (trial.eligible) {
    return (
      <div className="mx-4 mb-2 rounded-lg border border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950/50 p-3 flex items-center justify-between">
  ...
  // Show countdown for active trials
  if (trial.isTrialing && trial.daysRemaining > 0) {
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 text-sm">
          <Clock className={`h-4 w-4 ${urgent ? "text-amber-600 dark:text-amber-400" : "text-green-600 dark:text-green-400"}`} />
