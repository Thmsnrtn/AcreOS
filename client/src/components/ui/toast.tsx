...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed top-0 right-0 z-toast flex max-h-[100dvh] w-full flex-col-reverse p-4 sm:top-4 sm:right-4 sm:flex-col-reverse md:max-w-[420px]",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      // pointer-coarse: touch has no hover to reveal the button and no
      // precision for a 24px target — always visible, 44px tap area.
      "absolute right-2 top-2 rounded-md p-1 pointer-coarse:p-3.5 pointer-coarse:right-0.5 pointer-coarse:top-0.5 pointer-coarse:opacity-100 text-foreground/50 opacity-0 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600",
      className
    )}
    toast-close=""
