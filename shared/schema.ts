...
// ============================================
// PROPOSED CHANGES (Rosy River agent outbox)
// ============================================

// Outbox sitting in front of the evolution pipeline. The codebase monitor and
// multi-week planner write proposals here. Founder reviews via /founder/agent-queue
// and approved low-risk proposals get promoted into agentTasks → evolutionHistory.
// Auto-merge eligibility depends on riskTier + whether targetFiles touch
// client/src/**, server/db/schema*, server/routes/auth*, or billing/pricing.
export const proposedChanges = pgTable("proposed_changes", {
  id: serial("id").primaryKey(),
  pillar: text("pillar").notNull(),              // A_uiux | B_data | C_agentic | shared
  agent: text("agent").notNull(),                // codebase_monitor | multi_week_planner | telemetry_drift | etc.
  title: text("title").notNull(),
  rationale: text("rationale").notNull(),        // why this should ship — first-principles reasoning
  targetFiles: text("target_files").array().default([]).notNull(),
  proposedDiff: text("proposed_diff"),           // optional preview/sketch — full diff only after gauntlet stage 1
  riskTier: text("risk_tier").notNull(),         // low | medium | high — drives auto-merge eligibility
  autoMergeEligible: boolean("auto_merge_eligible").default(false).notNull(),
  estimatedCostUsd: numeric("estimated_cost_usd", { precision: 10, scale: 6 }),
  preMortem: text("pre_mortem"),                 // what could go wrong (Mindwired pattern)
  rollbackPath: text("rollback_path"),           // how to undo if it ships and regresses
  // Lifecycle
  status: text("status").notNull().default("proposed"),
  // proposed | awaiting_founder | promoted_to_gauntlet | gauntlet_passed | shipped | rejected | rolled_back | superseded
  gauntletProposalId: integer("gauntlet_proposal_id"),       // FK → agentTasks.id once promoted
  evolutionHistoryId: integer("evolution_history_id"),       // FK → evolutionHistory.id once gauntlet runs
  prNumber: integer("pr_number"),                            // GitHub PR # if PR was opened
  prUrl: text("pr_url"),
  // Founder decision
  founderDecision: text("founder_decision"),     // approve | reject | defer | null
  founderNotes: text("founder_notes"),
  ...
  index("proposed_changes_pillar_idx").on(table.pillar),
  index("proposed_changes_created_idx").on(table.createdAt),
]);
export const insertProposedChangeSchema = createInsertSchema(proposedChanges).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export type ProposedChange = typeof proposedChanges.$inferSelect;
export type InsertProposedChange = z.infer<typeof insertProposedChangeSchema>;

// ============================================
// AGENT COST LOG (Rosy River budget tracking)
// ============================================

// Per-action cost ledger across all pillars. Weekly aggregate drives the
// $50/week ceiling + $40 founder-alert threshold. Cost is denormalized
// from the OpenRouter / Anthropic response usage block at write time.
export const agentCostLog = pgTable("agent_cost_log", {
  id: serial("id").primaryKey(),
  pillar: text("pillar").notNull(),              // A_uiux | B_data | C_agentic | shared
  agent: text("agent").notNull(),                // which agent incurred the cost
  action: text("action").notNull(),              // proposal | gauntlet_stage | review | deploy | telemetry | planner
  model: text("model").notNull(),                // anthropic/claude-opus-4-7 | openai/gpt-4o | etc.
  inputTokens: integer("input_tokens").default(0).notNull(),
  outputTokens: integer("output_tokens").default(0).notNull(),
  costUsd: numeric("cost_usd", { precision: 10, scale: 6 }).notNull(),
  proposedChangeId: integer("proposed_change_id"),   // FK → proposedChanges.id (optional)
  evolutionHistoryId: integer("evolution_history_id"), // FK → evolutionHistory.id (optional)
  metadata: jsonb("metadata").$type<Record<string, unknown>>(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => [
  index("agent_cost_log_pillar_idx").on(table.pillar),
  index("agent_cost_log_created_idx").on(table.createdAt),
  index("agent_cost_log_agent_idx").on(table.agent),
]);
export const insertAgentCostLogSchema = createInsertSchema(agentCostLog).omit({
  id: true,
  createdAt: true,
});
export type AgentCostLog = typeof agentCostLog.$inferSelect;
export type InsertAgentCostLog = z.infer<typeof insertAgentCostLogSchema>;

// ============================================
// FOUNDER NOTIFICATIONS (Rosy River event feed)
// ============================================

// Continuous notification feed for agentic activity. Dedicated table (not
// folded into feedback_submissions) so the existing public/in-app feedback
// widget keeps its NOT NULL identity contract while agent events flow with
// a simpler schema. The /founder/notifications dashboard UNIONs this with
// feedback_submissions for a single timeline.
export const founderNotifications = pgTable("founder_notifications", {
  id: serial("id").primaryKey(),
  source: text("source").notNull(),
  // agent_propose | agent_ship | agent_rollback | founder_alert | budget_warning |
  // circuit_breaker_trip | gauntlet_fail | telemetry_drift | planner_weekly
  pillar: text("pillar"),                        // A_uiux | B_data | C_agentic | shared | null
  severity: text("severity").notNull().default("info"), // info | warning | critical
  title: text("title").notNull(),
  body: text("body"),
  // Optional cross-references for one-click drill-down from the feed
  proposedChangeId: integer("proposed_change_id"),
  evolutionHistoryId: integer("evolution_history_id"),
  prNumber: integer("pr_number"),
  prUrl: text("pr_url"),
  metadata: jsonb("metadata").$type<Record<string, unknown>>(),
  // Founder triage
  status: text("status").notNull().default("unread"), // unread | read | dismissed | actioned
  readAt: timestamp("read_at"),
  dismissedAt: timestamp("dismissed_at"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => [
  index("founder_notifications_status_idx").on(table.status),
  index("founder_notifications_source_idx").on(table.source),
  index("founder_notifications_created_idx").on(table.createdAt),
]);
export const insertFounderNotificationSchema = createInsertSchema(founderNotifications).omit({
  id: true,
  createdAt: true,
});
export type FounderNotification = typeof founderNotifications.$inferSelect;
export type InsertFounderNotification = z.infer<typeof insertFounderNotificationSchema>;
