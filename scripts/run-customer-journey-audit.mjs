#!/usr/bin/env node
/**
 * KRIEGER — Customer Journey Audit Runbook.
 *
 * Driver for the playwright.customer-journey.config.ts suite. Used by:
 *   1. .github/workflows/customer-journey-audit.yml (every 6h cron).
 *   2. Solene for manual triggers ("run a customer-journey audit").
 *
 * Behavior:
 *   - Runs the Playwright suite against the target URL.
 *   - Parses the JSON report (tests/e2e-customer-journey/results/playwright-report.json).
 *   - Writes a per-run structured summary:
 *       tests/e2e-customer-journey/results/<ISO_DATE>.json
 *   - Writes a per-run markdown call-out:
 *       docs/internal/customer-journey-results-<ISO_DATE>.md
 *     with a green/amber/red per viewport × theme table.
 *   - Exits non-zero if any viewport is RED so the CI workflow fails
 *     and the GH issue auto-opener fires.
 *
 * Usage:
 *   TARGET_URL=https://acreos.io node scripts/run-customer-journey-audit.mjs
 *   TARGET_URL=https://acreos.io node scripts/run-customer-journey-audit.mjs \
 *     --project=mobile-iphone-12-dark
 *
 * Flags:
 *   --project=<name>   restrict to a single project (skips matrix expansion)
 *   --theme=<l|d>      shorthand for --project filtering (light|dark suffix)
 *   --no-write         skip writing summary files (dry-run / debug)
 *   --skip-run         parse the existing JSON report only (use after a
 *                      separate Playwright invocation)
 *
 * Exit codes:
 *   0 — all viewports GREEN (or AMBER with no hard reds).
 *   2 — at least one viewport RED.
...
function log(...parts) {
  ...
  console.log("[customer-journey-audit]", ...parts);
}

function fatal(code, msg) {
  // eslint-disable-next-line no-console
  console.error("[customer-journey-audit] FATAL:", msg);
  process.exit(code);
}
...
if (!targetUrl && !flags["skip-run"]) {
  fatal(
    ...
    "TARGET_URL is required (or pass --skip-run to parse an existing report). " +
      "Example: TARGET_URL=https://acreos.io node scripts/run-customer-journey-audit.mjs",
  );
}
...
if (!flags["skip-run"]) {
  ...
  const pwArgs = [
    "playwright",
    "test",
    "--config=playwright.customer-journey.config.ts",
  ];
  if (flags.project) {
    pwArgs.push(`--project=${flags.project}`);
  } else if (flags.theme) {
    // --theme=light → all -light projects; --theme=dark → all -dark.
    pwArgs.push(`--grep-invert=`); // noop; Playwright filters by project
    log(
      "note: --theme is a shorthand hint; filter via --project=*-{light|dark}",
    );
  }
  ...
  // We DO NOT exit on Playwright's exit code here — we parse the JSON
  // report and synthesize the audit summary first, then exit based on
  // the per-viewport classification.
}
...
//   amber   — at least one test skipped (e.g. VOYAGE_API_KEY missing) but no
//             failures; OR a test passed only on retry.
//   red     — at least one test failed (final status).
// ────────────────────────────────────────────────────────────────────────────

/**
 * Walk the Playwright JSON report.
 * Suite shape: { suites: [ { specs: [ { tests: [ { results: [...] } ] } ] } ] }
 * Each spec.test has one entry per project (via projectId / projectName).
 */
function classify(report) {
  const perProject = {}; // projectName → { specs: [{ title, status }] }
    ...
    if (Array.isArray(suite.specs)) {
      for (const spec of suite.specs) {
        for (const t of spec.tests ?? []) {
          ...
          // Final result is the latest one (after retries).
...
if (!flags["no-write"]) {
  ...
  const mdOut = join(DOCS_DIR, `customer-journey-results-${isoDate}.md`);
  const mdLines = [];
  mdLines.push(`# Customer Journey Audit — ${summary.runAt}`);
  mdLines.push("");
  ...
  mdLines.push("");
  mdLines.push("## Per-viewport classification");
  mdLines.push("");
    ...
    const icon =
      c.classification === "red"
        ...
        : c.classification === "amber"
          ...
          : "🟢";
    mdLines.push(
      `| \`${proj}\` | ${icon} ${c.classification} | ${c.counts.green} | ${c.counts.amber} | ${c.counts.red} |`,
    );
  }
  mdLines.push("");
...
// ────────────────────────────────────────────────────────────────────────────
// Exit code drives the CI workflow's pass/fail signal.
// ────────────────────────────────────────────────────────────────────────────

if (totalReds > 0) {
  log(`exiting 2 — ${totalReds} viewport(s) RED`);
  process.exit(2);
}
log("exiting 0 — no red viewports");
process.exit(0);
