...
export interface IStorage {
  ...
  getLeadActivities(organizationId: number, leadId: number, limit?: number): Promise<LeadActivity[]>;
  ...
  // Custom Field Values
  getCustomFieldValues(entityType: string, entityId: number, organizationId: number): Promise<(CustomFieldValue & { definition: CustomFieldDefinition })[]>; // organizationId REQUIRED — values are org-owned via their definition (2026-09-04)
  ...
  getExecutiveMetrics(orgId: number, dateRange: { startDate: Date; endDate: Date }): Promise<{
    totalRevenue: number;
    // Optional means NOT MEASURED — see analyticsRepo.getExecutiveMetrics.
