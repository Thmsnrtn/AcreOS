...
export async function registerEnhancementRoutes(app: Express) {
  ...
  // The auth-config self-diagnostic MOVED to server/routes.ts (the pre-Clerk
  // health-probe block, next to /api/healthz): registered here it was shadowed
  // into a 404 by the earlier GET /api/health/:service wildcard, and it must
