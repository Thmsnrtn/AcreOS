...
export async function registerRoutes(
  ...
  // The inline /api/tasks CRUD surface (GET list, dashboard-summary, GET/:id,
  // POST, PUT/:id, DELETE/:id, POST/:id/complete, POST/process-recurring) was
  // DEFINED HERE and is DELETED (2026-08-27): registerCRMExtrasRoutes(app) runs
  // earlier in this function and registers the same eight paths, so Express
  // never reached these — production has been serving routes-crm-extras.ts the
  // whole time (lint:route-shadowing found all eight). dashboard-summary, the
  // one block with no live twin, moved to routes-crm-extras.ts above /:id,
