...
export async function generatePortfolioPdf(data: PortfolioSummaryData): Promise<Buffer> {
  ...
  // Footer with trust score
  let footerText = `Generated by ${data.orgName} — AcreOS`;
  if (data.organizationId) {
    try {
      const trust = await computeInvestorTrustScore(data.organizationId);
      footerText = `Generated by ${data.orgName} — AcreOS Verified Investor (Trust Score: ${trust.total})`;
    } catch (err) {
      logger.warn("Trust score unavailable for PDF footer", { organizationId: data.organizationId });
    }
  }
  doc.setFontSize(7.5);
  doc.setFont("helvetica", "italic");
  doc.setTextColor(160, 160, 160);
  doc.text(footerText, margin, 10.5);
...
export async function generateCashFlowPdf(data: CashFlowPdfData): Promise<Buffer> {
  ...
  // Footer with trust score
  let cashFlowFooter = `Generated by ${data.orgName} — AcreOS. Projections are estimates only.`;
  if (data.organizationId) {
    try {
      const trust = await computeInvestorTrustScore(data.organizationId);
      cashFlowFooter = `Generated by ${data.orgName} — AcreOS Verified Investor (Trust Score: ${trust.total}). Projections are estimates only.`;
    } catch (err) {
      logger.warn("Trust score unavailable for cash flow PDF footer", { organizationId: data.organizationId });
    }
  }
  doc.setFontSize(7.5);
  doc.setFont("helvetica", "italic");
  doc.setTextColor(160, 160, 160);
  doc.text(cashFlowFooter, margin, 10.5);
