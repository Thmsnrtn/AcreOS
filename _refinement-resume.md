...
**Dialog form onSubmit 4 (180):** ✅ complete — founder MRR-goal dialog: removed onKeyDown="Enter" hack, wrapped in proper `<form id>`. Input gained type="number" inputMode="numeric" min={0} autoComplete="off" + aria-label. (commit `37440b5`)
**onKeyDown=Enter sweep (181):** ✅ complete — 2 more handler-on-Enter hacks replaced with proper `<form onSubmit>`: founder system-API-key inline edit (Save type="submit", Cancel type="button"; aria-label per provider), AgentDebatePanel proposition (Debate type="submit"; autoCapitalize="sentences"). Both gate mutate on !isPending. (commit `ed7fb71`)
**Raw button hardening (182):** ✅ complete — 5 raw `<button>` elements got type="button" + aria-label + focus-visible ring + aria-hidden on icon: seller-finance amortization toggle (also aria-pressed + aria-controls), ProductTour skip, pax-copilot dismiss-banner, founder-dashboard refresh-support, OnboardingWalkthrough skip. (commit `93efb28`)
**Raw button hardening 2 (183):** ✅ complete — founder-dashboard sweep: refresh-briefing (aria-label varies with isFetching), show-more list toggle (aria-expanded), autopilot status bar (aria-expanded + aria-label), health-status filter chips (aria-pressed), and DASHBOARD_TABS gained full ARIA tablist contract (role="tablist" + role="tab" + aria-selected + aria-controls). (commit `ffe4a2b`)
**Raw button hardening 3 (184):** ✅ complete — 8 more buttons across 3 surfaces: founder scrollTo section nav (wrapped in `<nav aria-label>`, aria-current per item), marketplace copy-link (aria-label per listing), field-scout view tabs (role="tab" + aria-selected), inspection-checklist toggle (aria-expanded), voice-record FAB pair (aria-label start/stop). (commit `b942400`)
**Raw button hardening 4 (185):** ✅ complete — 5 surfaces: field-scout lead-row select (aria-label per lead) + quick-add FAB (aria-label), reseller-dashboard preview pill (tabIndex=-1 + aria-hidden + pointer-events-none, mockup), deal-inbox email-thread toggle (aria-expanded + focus-visible upgrade), tier-upgrade-panel monthly/annual switch (full ARIA switch contract: role="switch" + aria-checked). (commit `6f0ab4f`)
**Raw button hardening 5 (186):** ✅ complete — 7 surfaces: portfolio-optimizer stress-scenario picker (aria-pressed), pax-copilot-rail collapsed expand toggle (aria-label with unread count + aria-expanded), conversation rows (aria-current + aria-label per conversation), New-conversation, results-banner CTA, observation actions (Handle/Dismiss with aria-label naming the observation). (commit `1670902`)
...
**BulkActionBar a11y (208):** ✅ complete — converted outer fixed div to `<aside aria-label>` with dynamic count+entity, count Badge aria-live="polite", Clear button gained aria-label + X icon aria-hidden. (commit `e92ad52`)
...
**founder creative-edit + AI-reply (227):** ✅ complete — final 4 raw labels in founder-dashboard converted to `<Label htmlFor>` with per-row unique ids: copy-edit Headline/Primary-text/Description (also gained native maxLength matching slice limit) and AI-drafted reply Textarea (also autoCapitalize=sentences). founder-dashboard now has 0 raw labels. (commit `624deee`)
...
**email-compose-sheet shadcn Label + form-onSubmit (232):** ✅ complete — 5 raw `<label>` → shadcn `<Label>`; `<form id="email-compose-form" onSubmit>` + `<Button type="submit" form>` so Enter sends; Cancel/CC-BCC toggle now `type="button"`; Subject + Message autoCapitalize=sentences; ellipsis canonicalized ("Sending...", "Write your message...", "Email subject"); Mail + Send icons aria-hidden; "Send Email" → "Send email". (commit `f6e4fad`)
**deal-inbox reply composer (233):** ✅ complete — 3 raw `<label>` → shadcn `<Label>` on To/Subject/Message; `<form id="deal-inbox-reply-form" onSubmit>` + `<Button type="submit" form>`; Subject + Message autoCapitalize=sentences; "Send Reply" → "Send reply"; ellipsis canonicalized ("Sending...", "Write your reply..."); Cancel `type="button"`, Reply icon aria-hidden. (commit `956df25`)
**campaigns-content raw label sweep (234):** ✅ complete — 8 raw `<label>` → shadcn `<Label>` (Campaign name, Schedule, Template radiogroup, Blind-offer formula, Recipients, Subject, Content, Budget). Subject Input + Content Textarea gain autoCapitalize=sentences. campaigns-content.tsx now has 0 raw `<label>` elements. (commit `e3d5e09`)
**portfolio-health-card invalid markup + money-precision (235):** ✅ complete — wouter `<Link>` renders `<a>`; nested `<button>` was invalid HTML. Removed 2 inner buttons; classes moved to `<Link>` directly. Tax banner gained `<aside aria-label="Year-end tax optimization">` landmark. usd({noCents}) replaces bare `$${n.toLocaleString()}`. Sentence-case: "View Full Health"/"Year-End Tax Optimization"/"Needs Attention". Ellipsis "calculating..." → "calculating…". AlertTriangle + TrendingUp icons aria-hidden. (commit `f9feaa0`)
**quick-capture-fab DialogDescription + onError + wouter nav (236):** ✅ complete — added sr-only `<DialogDescription>` to silence Radix a11y warning; onError handler with canonical `"Couldn't read the sign"` toast (capture failure was previously silent); `window.location.href` full-reload → wouter `setLocation` (SPA state preserved); smart quotes around "For Sale". (commit `a94c030`)
**banners SPA-nav lens (237):** ✅ complete — trial-banner.tsx (2 hits) and neighbor-outreach.tsx (1 hit) had `window.location.href` full-reload nav for internal paths, blowing React Query cache. Routed through wouter `setLocation`. trial-banner gained `<aside aria-label>` landmark on both branches (dynamic label including days remaining), Sparkles/Clock icons aria-hidden, sentence-case ("Start Trial"/"Upgrade Now"/"Enrich This Property"). (commit `e636c7c`)
**freedom-meter SPA-nav (238):** ✅ complete — 3 `window.location.href` hits in freedom-meter.tsx (`/blind-offer-wizard` ×2, `/dunning`) routed through wouter `setLocation`. Empty-state "Find your first deal" CTA, "Add your next note" Start button, and "Manage late payments" View button now SPA-navigate without losing React Query cache + scroll. (commit `58198a5`)
**pax SPA-nav (239):** ✅ complete — Pax InsightsTabContent had 3 `window.location.href` hits to `/leads/{id}` ×2 and `/deals/{id}`. Routed through `setLocation`: Follow-up CTA on dormant warm leads, Review-now on expiring offers (role=alert), View on motivated callers. Eliminates full reloads on the most-used Pax workflow handoffs. (commit `3f23edb`)
**maps + blind-offer-wizard SPA-nav (240):** ✅ complete — maps.tsx geolocate "add property here" flow had 3 `window.location.href` to `/properties?addAtLat=…` (success/fallback/exception paths) — all dropped users into a full reload mid-flow; routed via `setLocation`. blind-offer-wizard StepLetter "Launch campaign" CTA also routed via `setLocation` to `/direct-mail-campaigns`. (commit `9715f22`)
**raw anchor → wouter Link sweep (241):** ✅ complete — 5 raw `<a href="/spa-path">` anchors converted to wouter `<Link>` across campaigns-content (CSV helper "Leads page"), founder-home ("See all →"), integrations-health ("Settings → Notifications"), team-inbox (Button asChild "Upgrade plan"), founder-dashboard ("Open integrations dashboard" + sentence-case). (commit `7d4dca0`)
**raw anchor sweep finish (242):** ✅ complete — 6 more `<Button asChild>+<a href>` patterns → Link across pax.tsx (×2: Act CTA + Go to integrations), drip-sequences (New sequence CTA), team-dashboard (per-row Message), conversation-tray (Upgrade plan + sentence-case), calendar-widget (Connect Google Calendar). All SPA-internal raw `<a href>` in client are now wouter `<Link>`; remaining anchors are external (Stripe portal, mailto:, tel:) or intentional `target=_blank`. (commit `09a04ba`)
**analytics-content chart a11y (243):** ✅ complete — pivot to chart-a11y lens after target=_blank lens came up clean. analytics-content.tsx had 5 ResponsiveContainer charts with no a11y wrapper. Each wrapped in `<div role="img" aria-label={computed-summary}>`: MRR forecast (current/projected/growth), revenue line chart (data-point count), lead-sources pie (source→count), deal-pipeline bar (stage→value), avg-days-per-stage horizontal bar (stage→days + bottleneck). Labels reflect live data. (commit `9954728`)
**attribution + cohort chart a11y (244):** ✅ complete — attribution-analytics 2 charts (conversions-by-channel pie, conversions-by-touch-number bar) and cohort-analytics 1 chart (conversion-rates by segment, multi-bar) all gained `role="img"` + computed live-data aria-labels. (commit `f0d47b8`)
**chart-a11y sweep finish (245):** ✅ complete — exhaustive audit of pages/* and components/*: ai-cost-dashboard (cost-by-provider bar) + pipeline-velocity (stage-velocity bar with stalled flag) wrapped with computed labels. All Recharts ResponsiveContainer instances in client now sit inside `<div role="img" aria-label>`. Across slices 243–245: 12 charts wrapped + ~20 verified pre-wrapped. (commit `b3bb411`)
**MobileBottomNav a11y (246):** ✅ complete — table-a11y lens verified clean (15/15 files have `<th scope>`); pivoted to nav-a11y. MobileBottomNav had visual active state but no SR signal: `<nav>` gained `aria-label="Mobile navigation"`, each Link gets `aria-current={isActive ? "page" : undefined}`, item icons + MoreHorizontal aria-hidden, More button gets `type=button` + `aria-expanded` + `aria-haspopup="dialog"`, focus-visible rings added. (commit `65fd744`)
**landing waitlist hardening (247):** ✅ complete — first user-facing form on the marketing landing had a raw `<input type=email>` with no Label, no autoComplete, no inputMode, no autoCapitalize/autoCorrect/spellCheck. Replaced with shadcn `<Input>` + sr-only `<Label>` + full mobile-keyboard stack so password managers (1Password, iCloud Keychain) auto-fill the saved email. Sentence-case "Join Waitlist" → "Join waitlist". (commit `aae291f`)
**form-a11y gaps (248):** ✅ complete — reseller-dashboard branding panel: "Support Email" Input had only type=email; added full mobile-keyboard stack (inputMode/autoComplete/autoCapitalize/autoCorrect/spellCheck), Font family + Footer text Label htmlFor + ids, sentence-case "Support Email"/"Font Family"/"Footer Text"/"Save Branding". request-signatures-dialog: per-signer Labels (Name/Email/Role) missing htmlFor — added per-row unique ids `signer-name-{i}` / `signer-email-{i}` / `signer-role-{i}`. (commit `b6d2529`)
**atlas + legal-intelligence money-precision (249):** ✅ complete — prefers-reduced-motion verified covered globally via App.tsx `<MotionConfig reducedMotion="user">`. atlas.tsx (only meaningful page missing useDocumentTitle) now has it. legal-intelligence-card tax-lien banner had bare `$${n.toLocaleString()}` for delinquent-tax amount — money-precision bug on a compliance/acquisition surface; routed through usd() per dunning-manager precedent. (commit `c1479eb`)
**maps property valuations money-precision (250 — milestone):** ✅ complete — maps.tsx had 5 bare `$${n.toLocaleString()}` hits on the highest-stakes property-valuation surface (land-investor acquisition decisions hinge on these): estimated value, price-per-acre, chart tooltip, assessed value, annual taxes. All routed through `usd(n, { noCents: true })` (whole-dollar property valuations, cents-safe). Added tabular-nums on the live displays. Sentence-case "Assessed Value"/"Annual Taxes". The elite refinement arc has now run 250 slices. (commit `d3eb9b7`)
**ForecastPanel + goals money-precision (251):** ✅ complete — founder ForecastPanel: 5 fields (current MRR, projected, low/high bounds, milestone target, avg revenue per customer) routed through usd({noCents}); tabular-nums on every money display; loading skeletons gain role=status + aria-busy; "6-Month Projection" → "6-month projection"; ellipsis on "Calculating…"; 4 lucide icons aria-hidden. goals.tsx fmt() sub-$1k branch routes through usd() with isInteger-aware noCents so $99.99 goals don't round to $100. (commit `cad54c5`)
**content-generation + property-chat money-precision (252):** ✅ complete — content-generation.tsx (4 hits: Facebook/Craigslist/social listing prices, deal case-study purchase+sale+profit) and property-analysis-chat.tsx (3 hits: Market/List Value injected into AI prompt + formatCurrency helper) all routed through `usd(n, { noCents: Number.isInteger(n) })`. Text the user publishes externally must reflect actual price including cents. (commit `051e37f`)
**tax-researcher + market-data + founder-settings money-precision (253):** ✅ complete — tax-researcher fmt() sub-$1k branch routed through usd() with isInteger noCents (compact M/k retained for ≥1k); market-data state-card avg-$/acre routed through usd() (preserves tabular-nums); founder-settings ConfigRow cents-unit displayValue routed through `usd(value/100)`. (commit `a58919b`)
**borrower-statement-pdf money-precision (254):** ✅ complete — highest-stakes money surface in the app: borrower-facing loan-statement PDF with 9 fields rendered via bare `$${n.toLocaleString()}`. Account summary (Original Principal/Current Balance/Monthly Payment), Payment summary (Total Paid/Principal Paid/Interest Paid), per-row history (Amount/Principal/Interest × N). All routed through `usd()` — borrowers reconcile against bank statements + amortization, so cents must match to the penny. (commit `33800b0`)
**founder-MRR + model-training + cash-flow-waterfall money-precision (255):** ✅ complete — founder-dashboard.tsx 5 MRR fields (goal-saved toast, MRR stat card, customer-health header total + at-risk, per-tier waterfall) routed through usd() with isInteger noCents. model-training fmt() helper sub-$1k branch routed. cash-flow-waterfall fmt$() Math.round'd display now uses usd({noCents:true}) — comment in code had already flagged "money-precision rule still applies." (commit `a6fde27`)
**command-center + avm + field-scout money-precision (256):** ✅ complete — command-center "Price Recommendation Ready" toast description routed through usd() (also sentence-case title); avm Recharts tooltip pricePerAcre formatter routed through usd(); field-scout per-lead assessedValue display routed through usd({noCents}) + tabular-nums. Money-precision lens fully covered across the client. (commit `ef61541`)
**aria-busy skeletons (257):** ✅ complete — pivot to aria-busy lens (only 3 files used it pre-slice). 5 widely-used widgets gained `role="status"` + `aria-busy="true"` + `aria-label="Loading X"` + sr-only "Loading…" text on their isLoading branches: portfolio-health-card, cash-flow-waterfall, ai-cost-dashboard, legal-intelligence-card, calendar-widget. cash-flow-waterfall also sentence-cased "Cash flow waterfall" + DollarSign icon aria-hidden. (commit `0f70741`)
**aria-busy continued (258):** ✅ complete — focus-list (Daily focus widget, sentence-cased), sequences-content (campaigns tab), deal-inbox (lead-detail email panel) — all three isLoading branches gain role=status + aria-busy + aria-label + sr-only "Loading…" + decorative-icon aria-hidden. (commit `14dce8b`)
**aria-busy continued pt3 (259):** ✅ complete — verified ListSkeleton component already has aria-busy/role=status (used widely); pax.tsx top-level had role=status. Hardened remaining naked branches: conversation-tray spinner, pax AiChatGuard naked Skeleton (wrapped), founder-dashboard auto-resolutions + scheduled-jobs skeleton blocks. (commit `3678ebf`)
**system-health a11y polish (260):** ✅ complete — sentence-case "System Health" + 4 overall-status strings ("All Systems Operational" → "All systems operational", etc.); Activity/Loader2/RefreshCw icons aria-hidden; loading container gained role=status + aria-busy + aria-label + sr-only "Loading…". (commit `7d389c5`)
**beta-feedback-widget hardening (261):** ✅ complete — `<div>` wrapper → `<aside aria-label="Beta feedback">` landmark; onClick submit → `<form id="beta-feedback-form" onSubmit>` + `<Button type="submit">` so Enter submits; floating "Send Feedback" h4 → `<Label htmlFor>` linked to Textarea (was unlabeled); Close button now `type="button"`; autoCapitalize=sentences; ellipsis canonicalized; X/Send/MessageSquare icons aria-hidden. (commit `45cbd88`)
**feedback-dialog polish (262):** ✅ complete — feedback-button.tsx already had Form + DialogDescription + Label htmlFor. Polish: removed unused MessageSquarePlus + Tooltip imports (FeedbackButton itself is a no-op shim); SelectItem sentence-case (Bug report/Feature request); Textarea placeholder "..." → "…" + autoCapitalize=sentences; submit "Sending..." → "Sending…", "Send Feedback" → "Send feedback". (commit `e9c85cd`)
**nps-dialog radiogroup (263):** ✅ complete — NPS 0-10 score selector was a flat row of unrelated `<Button>` (screen readers couldn't tell it was a single-select scale). Promoted to ARIA radiogroup: outer `role="radiogroup"` + `aria-labelledby={title-id}`, each score gets `role="radio"` + `aria-checked={selectedScore === i}` + `type="button"`. Textarea autoCapitalize=sentences; "Submitting..." → "Submitting…"; skip/submit explicit `type="button"`. (commit `fdb4aa9`)
**cancellation-dialog hardening (264):** ✅ complete — payment-impact surface. RadioGroup wasn't labelled (Label had no id) — added `Label id` + `RadioGroup aria-labelledby`; feedback Textarea had no Label — added sr-only Label htmlFor + autoCapitalize=sentences; cancelMutation had no onError (silent failure on critical action) — added canonical "Couldn't cancel subscription" toast with state-preserving reassurance; AlertTriangle + TrendingDown icons aria-hidden; "Processing..." → "Processing…". (commit `0eebafa`)
**credit-purchase-modal hardening (265):** ✅ complete — Card-grid was a clickable-div single-select with no keyboard access. Promoted to ARIA radiogroup: role="radiogroup" + aria-labelledby={DialogTitle-id}; each Card role="radio" + aria-checked + roving tabIndex + Space/Enter keyboard handler + focus-visible rings. onError toast "Purchase Failed" → canonical "Couldn't start purchase" with state-preserving reassurance. Sentence-case DialogTitle. Pending button shows "Starting…" + 3 icons aria-hidden. (commit `761bc0c`)
**tier-upgrade-panel hardening (266):** ✅ complete — sentence-case sweep ×9 ("Get Started"/"Start Free Trial"/"Most Popular"/"Current Plan"/"Compare Plans"/"AI Requests"/"BYOK Data Providers"/"Team Seats"/"Data Sources"); money fields (monthly/yearly) routed through usd({noCents}) + tabular-nums; FeatureCell Check/X icons gained `aria-label="Included"/"Not included"` + role="img" (SR users couldn't tell which features were in each tier); Sparkles aria-hidden; comparison table aria-label; PlanComparisonModal sr-only DialogDescription. (commit `d331eec`)
**pricing public-page money-precision (267):** ✅ complete — public marketing pricing page already well-built (FeatureValue uses sr-only + aria-hidden, role=region on table). Tier monthly + yearly money displays routed through `usd({noCents:true})` (first money numbers a prospective customer ever sees). Contact-us mailto: link gained explicit aria-label including recipient + intent ("Email support@acreos.io for enterprise pricing"). (commit `4034d72`)
**ApiKeySetup + mailto sweep (268):** ✅ complete — settings/ApiKeySetup (used for every external-API integration): onClick → form-onSubmit so Enter submits, "API Key" → "API key" sentence-case, password Input autoComplete=off + spellCheck=false (API keys must not be browser-suggested), aria-invalid + aria-describedby on validation error, error region role="alert", validating state shows sr-only "Validating…", 4 icons aria-hidden. data-export.tsx GDPR mailto: link gained aria-label. **Domain inconsistency flagged**: acreos.io (pricing/status) vs acreos.com (legal/privacy/data-export) — needs founder decision on canonical domain, not refactored. (commit `7b143d0`)
**TeamInviteCard hardening (269):** ✅ complete — single-invite onClick → form-onSubmit (Enter submits); raw `<textarea>` → shadcn `<Textarea>` + Label htmlFor + autoCapitalize/autoCorrect/spellCheck off (CSV addresses); "Pending invitations" Label promoted to `<h3>` (Label without htmlFor is invalid); per-row revoke X gained `aria-label="Revoke invitation to {email}"` (was silent); bulk-invite disclosure gained aria-expanded; 3 lucide icons aria-hidden; canonical toasts ("Could not" → "Couldn't" + state-preserving reassurance). (commit `cd23c84`)
**Toast canonical sweep (270 — milestone):** ✅ complete — final 3 "Could not X" → "Couldn't X" toast titles canonicalized: use-copy.ts (foundational clipboard hook), founder-decisions.tsx (decision-reversal recording), request-signatures-dialog (signing-flow critical-action). All destructive-variant toasts across the client now use "Couldn't X" titles with state-preserving descriptions. (commit `253ebbf`)
**img-alt audit + property-map polish (271):** ✅ complete — verified all `<img>` tags have alt attributes (lens fully covered). property-map.tsx loading overlay gained role=status + aria-busy + aria-label + sr-only "Loading…"; Loader2 + MapPin icons aria-hidden; "View Details" → "View details". (commit `058ddfb`)
**pwa-install-prompt hardening (272):** ✅ complete — outer `<div>` wrappers (×2 branches) → `<aside aria-label="Install AcreOS">` landmark. Download icon aria-hidden. iOS prose: inline `<Share>` icon gained aria-label="Share" + role="img" + explicit "Share button" text so SR users hear the iOS button name. X dismiss aria-label → "Dismiss install prompt"; Plus + X icons aria-hidden; non-iOS body text gained period. (commit `acee853`)
**floating-help-button polish (273):** ✅ complete — FAB gained `aria-keyshortcuts="Meta+Shift+? Control+Shift+?"` so SR users hear the documented Cmd+? shortcut; ExternalLink icon on "Open full help center" was misleading (it links to SPA route /help, not external) — swapped to BookOpen; SheetContent missing SheetDescription → added sr-only description; 3+ lucide icons aria-hidden. (commit `db84019`)
**live-demo-mode announcements (274):** ✅ complete — narration paragraph + status footer wrapped in role=status + aria-live=polite (SR users now hear demo commentary + pause/resume transitions); step-dot row promoted to role="img" + computed `aria-label="Step X of Y"` (was decoration-only); speed button aria-label includes current speed; "Live Demo Mode" sentence-cased; "Preparing to execute..." → "…"; 6 lucide icons aria-hidden; pulsing indicator dot aria-hidden. (commit `d01418f`)
**sms-conversation role=log (275):** ✅ complete — messages container promoted to `role="log"` + `aria-live="polite"` + aria-label so SR users hear incoming SMS as they arrive (15s poll). Loading skeleton gained role=status + aria-busy + sr-only "Loading…". DNC-blocked replacement promoted to `role="alert"` (TCPA compliance now interrupts SR with critical announcement). Input gains autoCapitalize=sentences; "Type a message..." → "…"; 6 lucide icons aria-hidden. (commit `83f35a6`)
**comment-thread role=log + shadcn Textarea (276):** ✅ complete — comment list promoted to role="log" + aria-live="polite" + aria-label (mirrors sms-conversation pattern); loading skeleton role=status + aria-busy; "Comments" span → `<h3>`; raw `<textarea>` → shadcn `<Textarea>` + sr-only Label htmlFor + autoCapitalize=sentences; "Add a comment..." → "…"; Loader2/Send icons aria-hidden. (commit `56f1cca`)
**floating-assistant role=log (277):** ✅ complete — finishes the live-thread role=log lens. Always-mounted AI assistant chat had silent message stream; ScrollArea inner wrapper promoted to role="log" + aria-live="polite" + aria-label="Assistant conversation". "Temporary Mode" badge sentence-cased; Ghost icon aria-hidden. Lens is now covered across: sms-conversation (275), comment-thread (276), property-analysis-chat (pre-existing), floating-assistant (277). (commit `e386bbc`)
**notification-center semantic list (278):** ✅ complete — flat `<div>` stack of notifications → semantic `<ul>`/`<li>` (list-none/p-0/m-0 keeps visual identical) + aria-label so SR users hear "list, N items"; unread indicator dot gained sr-only "Unread" text (was color/dot-only); type icons + empty-state Bell aria-hidden. (commit `6d22665`)
**activity-timeline a11y (279):** ✅ complete — loading skeleton role=status + aria-busy + sr-only "Loading…"; error state "Failed to load timeline" → canonical "Couldn't load timeline. Refresh the page or try again in a moment." + role="alert"; events `<motion.div>` → `<motion.ol>`/`<motion.li>` semantic ordered list (chronological timeline) with aria-label + list-none/p-0/m-0. (commit `c276993`)
**saved-views-selector polish (280 — milestone):** ✅ complete — Save-view dialog already had form-onSubmit + Label htmlFor. Polish: 7 lucide icons aria-hidden (ChevronDown/Eye×2/Plus/Settings/Star/Trash2); "Visible Columns" Label misused as section heading → promoted to `<h3>` + sentence-case. The 280-slice arc has driven canonical patterns across toasts/money/charts/tables/forms/aria-busy/anchors/role=log/semantic-lists/dropdowns. (commit `44501c6`)
**notes-import-dialog drop zone (281):** ✅ complete — clickable-div CSV drop zone (slice-7 anti-pattern, keyboard users couldn't trigger file picker) → `<button type="button">` with aria-label + focus-visible ring + same drag handlers; hidden `<input type=file>` moved out of button (invalid nesting); "Import Notes from CSV" sentence-cased; importing step gained role=status + aria-busy + aria-live; 3 icons aria-hidden. (commit `e6fffc8`)
**tax-delinquent-importer keyboard focus (282):** ✅ complete — sister CSV importer uses `<label>`-wraps-`<input>` (valid click pattern); `className="hidden"` made input unfocusable to keyboard users → switched to `sr-only` (in tab order, visually invisible). Wrapping `<label>` gained `focus-within:ring-2`. Other than that the file was well-built (Upload aria-hidden, Badge required asterisk aria-label, ul/li columns). (commit `51e13d1`)
**pax-knowledge + pax-project keyboard drop zones (283):** ✅ complete — two more clickable-div drop zones (slice-281 pattern). pax-knowledge-panel + pax-project-panel: drop-zone `<div>` → `<button type="button">` with aria-label + focus-visible + disabled-on-limit; hidden input → sr-only + aria-label; icons aria-hidden. Verified onboarding-v2 already uses role="button" + tabIndex + onKeyDown — already accessible. (commit `031cfdf`)
**leads + properties import keyboard focus (284):** ✅ complete — same fix as slice 282 for two more `<label>`-wrap-`<Input>` import dialogs. Switched `hidden` → `sr-only` on file inputs, added `focus-within:ring-2` to wrapping labels; properties.tsx Input gained missing aria-label + "5MB" → "5 MB." Verified floating-assistant + command-center programmatic-trigger pattern intentionally hidden (secondary action via Attach button). (commit `b905613`)
**comps-analysis sentence-case (285):** ✅ complete — file was already very well-built (aria-hidden, role=status, role=alert, sentence-case "Couldn't load comps" + Label htmlFor). Polish: "Location Data Required"/"Comps Data Unavailable"/"Try Again"/"Min Acreage"/"Max Acreage" sentence-cased. Local formatCurrency uses Intl maximumFractionDigits=0 (deliberate "no cents on per-acre" call) — left as-is. (commit `8ac39e3`)
**daily-deal-feed money + a11y (286):** ✅ complete — formatCurrency routed through usd({noCents: isInteger}) (was bare `$${val.toLocaleString()}` on a money-relevant deal-of-the-day surface where investors decide acquisition pricing); loading state gained role=status + aria-busy + sr-only "Loading…"; error Card promoted to role="alert" + AlertCircle aria-hidden. (commit `941480d`)
**toast sentence-case sweep (287):** ✅ complete — 11 Title-Case toast titles canonicalized across foundational hooks + page surfaces. use-api-retry.ts: "Session Expired"/"Request Failed"/"Data Refreshed"/"Refresh Failed" → sentence-case + "Couldn't X" for failures (everywhere-effect). queryClient.ts: "Usage Limit Reached"/"Session Expired" sentence-cased. ai-offer-generator: "Letter Generated"; deals.tsx: "Cannot Advance Stage" → "Couldn't advance stage" + "Analysis Saved"; command-center: "Insufficient AI Credits"/"Trial Token Available"/"Upgrade Required" → sentence-case. (commit `bd8695d`)
**toast sentence-case pt2 (288):** ✅ complete — 9 more Title-Case toast titles. document-generator: "Document Generated"/"Generation Failed" → "Couldn't generate document". mail-settings-content: "Verification Initiated"/"Verification Failed" → "Couldn't verify address". property-map: 5 toasts ("Comparables Found"/"Map Exported"/"Copied to Clipboard"/"Export Failed"×2/"Copy Failed") → sentence-case + "Couldn't X" canonical for failures with retry suggestions. (commit `45935ed`)
**toast sentence-case pt3 (289):** ✅ complete — final 4 Title-Case toast titles: mail-settings "Return Address Created"; property-map "No Property Selected" (×2) + "Nearby Parcels Found". Verified remaining `title:` Title-Case strings are walkthrough/feature/playbook step titles (intentional Title Case headings, not toasts). **24 toasts canonicalized total across slices 287–289**. (commit `fed9d7b`)
**import-export polish (290 — milestone):** ✅ complete — toast titles already canonical ("Couldn't preview file"/etc), dropzone uses explicit Browse Button (no clickable-div anti-pattern). Polish: Upload/Download/Database on 3 tab triggers + Upload + Loader2 on dropzone area aria-hidden (5 icons). The 290-slice arc has reached coverage of the major canonical-pattern lenses: toasts, money, charts, tables, forms, aria-busy, anchors, role=log, semantic lists, file-input keyboard focus, decorative icon aria-hidden. (commit `f239946`)
**pax-copilot-rail role=log (291):** ✅ complete — fifth and final chat surface gains role=log. Pax right-side rail message ScrollArea promoted to role="log" + aria-live="polite" + aria-label="Pax conversation"; loading-history skeleton gained role=status + aria-busy + sr-only "Loading…"; Sparkles empty-state icon aria-hidden. Live-thread role=log lens fully covered: sms-conversation (275), comment-thread (276), property-analysis-chat (pre-existing), floating-assistant (277), pax-copilot-rail (291). (commit `6a77d52`)
**team-general-channel role=log (292):** ✅ complete — sixth chat-thread surface gains role=log. Loading skeleton role=status + aria-busy + sr-only "Loading…"; messages container role=log + aria-live + aria-label="Team chat"; Input autoCapitalize=sentences; placeholder "..." → "…"; Hash + Send icons aria-hidden. (commit `1b68fe5`)
**agent-team-chat role=log (293):** ✅ complete — seventh chat-thread surface gains role=log (Sophie/Forge/Atlas founder agent chat). Messages area role=log + aria-live + aria-label; "Thinking..." pending indicator gained role=status + aria-live + Loader2 aria-hidden + ellipsis canonicalized; Textarea autoCapitalize=sentences + aria-label; placeholder "..." → "…" in both branches. (commit `72cf4ff`)
**agent-debate-panel a11y (294):** ✅ complete — founder debate-to-decision surface. Loading skeleton wrapped in role=status + aria-busy + sr-only "Loading…"; "Agent Debates" → "Agent debates" sentence-case; CEO-decision reasoning Textarea gained aria-label + autoCapitalize=sentences + "…" ellipsis (was unlabeled); 4 lucide icons aria-hidden. (commit `200d8e4`)
**pax-connector-panel hardening (295):** ✅ complete — connect dialog used for every external integration (Stripe/Twilio/SendGrid/Regrid). Sheet loading state gained role=status + aria-busy + sr-only "Loading…"; DialogContent gained sr-only DialogDescription; dialog body `<div>` → `<form id="connect-connector-form" onSubmit>` so Enter submits; secret credential Inputs gain autoComplete=off + spellCheck=false (matches slice 268 ApiKeySetup pattern — API keys must NOT be browser-suggested); Cancel `type="button"`; Connect Button shows "Connecting…" during pending. (commit `7778cc9`)
**integrations-settings hardening (296):** ✅ complete — sister Settings → Integrations surface for Twilio/Lob/SendGrid/etc. Loading state role=status + aria-busy + sr-only "Loading…"; Configure dialog body → form-onSubmit (Enter submits); API key Input + additionalFields Inputs (Account SID, etc.) gain autoComplete=off + spellCheck=false; Cancel `type="button"`; Eye/EyeOff/Loader2/Provider-Icon aria-hidden; "Save Credentials" → "Save credentials"; "Saving…" pending state. (commit `24fb9ad`)
**email-domains-settings polish (297):** ✅ complete — "Add Domain" → "Add domain"; loading text gained role=status + aria-busy + aria-live + ellipsis; "Set Default" → "Set default" + Star aria-hidden; per-row Delete button aria-label became dynamic `Delete domain ${domain.domain}` (was generic across all rows — SR users couldn't tell which they'd delete); Plus + Trash2 icons aria-hidden. Form already had Label htmlFor + form-onSubmit + autoCapitalize/spellCheck hardening. (commit `3e23b02`)
**dynamic per-row aria-label sweep (298):** ✅ complete — workflows-settings-tab: Edit/Delete "workflow" → `Edit/Delete workflow ${wf.name}`; phone-numbers-settings: Delete `phone number ${formatted}` + "Set Default" sentence-cased; ab-tests-content: Delete `test ${test.name}`. Trash2/Edit2/Star icons aria-hidden. (commit `4db07e1`)
**dynamic per-row aria-label pt2 (299):** ✅ complete — email-settings-content (×2 instances) → `Delete email identity ${identity.fromEmail}`; pax-tasks-settings-tab → `Delete scheduled task ${task.name}`; command-center conversation list → `Delete conversation ${conv.title}`. All Trash2 + History icons aria-hidden. (commit `e0a46ef`)
**dynamic per-row aria-label pt3 (300 — milestone):** ✅ complete — finishes the sweep started in slice 297. deals.tsx Delete deal → `Delete ${county}, ${state}` (matches visible heading); properties.tsx Delete property → `Delete property ${county}, ${state}`; founder-dashboard GIS endpoint Delete → `Delete endpoint ${county}, ${state}` + title attr tightened. **9 surfaces total** now have dynamic per-row delete aria-labels (slices 297–300) — SR users acting on a list now hear which specific item the destructive action targets, eliminating "did I delete the right one?" failure mode. (commit `226756b`)
**onboarding-walkthroughs polish (301):** ✅ complete — founder/OnboardingWalkthrough 3 raw nav `<button>` (Back/Skip/Next) gained `type="button"` + focus-visible rings + ChevronLeft/Right aria-hidden. onboarding/ProductTour gained Escape-key dismiss handler (was X-button only) + backdrop `<div onClick={onSkip}>` gained `aria-hidden="true"`. (commit `c1fa8cf`)
**founder-walkthrough role=dialog (302):** ✅ complete — custom modal overlay had no role/aria-modal (assistive tech treated as page section) + no Escape handler. Added Esc-to-dismiss useEffect; motion.div wrapper now has role="dialog" + aria-modal="true" + aria-label="Founder onboarding walkthrough". (commit `3ba181a`)
**new-item-menu role=dialog + hardening (303):** ✅ complete — Cmd+N create-new picker custom modal. Wrapper motion.div promoted to role="dialog" + aria-modal + aria-labelledby; backdrop aria-hidden; Esc-to-dismiss handler added; "Create New" h2 sentence-cased; close X button gained type=button + aria-label + focus-visible ring + X aria-hidden; all 5 option buttons gained type=button + focus-visible ring + decorative icon aria-hidden. (commit `382a3cb`)
**info-card icon polish (304):** ✅ complete — shared InfoCard wrapper div for icon gained aria-hidden (decorative); ChevronUp/Down in Show more/less toggle aria-hidden (visible text label is the SR target). disclaimer-banner + usage-limit-banner verified already well-built (role=note/alert + aria-live + dynamic aria-label + icons aria-hidden). (commit `5aa68e6`)
**bulk-action-bar specific titles (305):** ✅ complete — AlertDialog confirmation title generic "Are you sure?" → dynamic `${action.label} ${count} ${entity}${plural}?` (e.g. "Delete 5 leads?" / "Archive 1 deal?") so SR users hear exactly what they're confirming. Default confirmMessage plural fix from "leads(s)" → conditional "leads" / "lead". (commit `f4bc326`)
**template-editor + audit cleanup (306):** ✅ complete — per-field Remove button aria-label "Remove field" → dynamic `Remove field ${field.name || #N+1}`; Trash2 aria-hidden; template-content placeholder "…". Verified all AlertDialogTitle instances across codebase are already specific (founder-home, job-health, daily-deal-feed); ConfirmDialog shared component well-built; safe-bulk-delete-dialog + campaign-variants-panel verified clean. (commit `9f1d73c`)
**workflow-builder per-row index-aware aria-labels (307):** ✅ complete — Remove condition/Remove action/Move action up/Move action down generic aria-labels → `${verb} ${condition|action} ${index+1} ${direction?}` so SR users navigating a multi-action workflow hear which row. 4 lucide icons aria-hidden (Trash2, ChevronUp, ChevronDown). (commit `e7b73fb`)
**keyboard-shortcuts-dialog semantic lists + sentence-case (308):** ✅ complete — DialogTitle "Keyboard Shortcuts" → "Keyboard shortcuts" (sentence-case); Keyboard icon aria-hidden; Navigation/Actions wrapper `<div>` → `<section aria-labelledby>` with stable heading IDs; inner shortcut row stacks `<div>` → `<ul>/<li>` with list-none/p-0/m-0 (semantic, identical visuals). (commit `9ebb82a`)
**cost-confirmation-modal canonical sweep (309):** ✅ complete — DialogTitle/labels Title Case → sentence-case ("Confirm action cost", "Estimated cost", "Current balance", "Balance after", "Add credits"); 4 lucide icons aria-hidden (CreditCard/Loader2/AlertTriangle/Plus); estimate spinner gets role=status + aria-live + aria-busy + sr-only "Estimating cost…"; insufficient-credits notice gets role=alert; money display switched from bare `${(n/100).toFixed(2)}` to `usd(n/100)` canonical helper with tabular-nums. (commit `8c473d9`)
**quick-actions-menu role=menu + aria-expanded (310):** ✅ complete — mobile FAB popout: action labels "New Lead/Property/Deal" → sentence-case; popout `<div>` → `<ul role=menu aria-label>` with id targeted by toggle's aria-controls; inner stack `<button>` wrapped in `<li role=none>`; buttons get role=menuitem + type=button + focus-visible ring; trailing action icons aria-hidden; toggle FAB gets type=button + aria-expanded + aria-haspopup=menu + aria-controls + dynamic aria-label "Open/Close quick actions"; X/Plus icons aria-hidden. (commit `bedc238`)
**request-signatures-dialog role=status + ul/li + aria-busy (311):** ✅ complete — success "Signing links ready" notice wrapped in role=status + aria-live=polite (SR users hear async link generation); per-signer link results `<div>` → `<ul aria-label="Signing links">/<li>` with list-none/p-0/m-0 (semantic, identical visuals); 5 lucide icons aria-hidden (CheckCircle2, Trash2, Plus, Loader2, Send); Remove signer aria-label upgraded `${i+1}` → `${s.name.trim() || i+1}` (dynamic w/ row fallback); submit gets aria-busy + label flips "Generate signing links" → "Generating…" while pending. (commit `d140014`)
**onboarding-modal sentence-case + tablist dots (312):** ✅ complete — step titles "Manage Your Leads"/"Track Your Properties"/"Close Deals"/"Stay Notified" → sentence-case; "Get Started" CTA → "Get started"; StepIcon + ArrowRight aria-hidden; step nav dots `<button>` → `<button type=button role=tab aria-selected aria-label="Step N of M: Title">` inside `<div role=tablist aria-label="Onboarding steps">` with focus-visible ring; Skip + Next gain type=button + aria-busy. (commit `b223e50`)
**compliance-settings full sweep (313):** ✅ complete — major surface (621L) hardened: card/section/tab titles + stat tile labels + policy item labels + SelectItem labels Title Case → sentence-case throughout; 11 lucide icons aria-hidden; tab buttons get type=button + role=tab + aria-selected inside role=tablist aria-label; consent-rate progress bar promoted to role=progressbar w/ aria-labelledby + aria-valuenow/min/max; retention rows: Input + Switch each gain dynamic aria-label "${item.label} retention…"; htmlFor/id pairing across all filter selects + purge form inputs; tabular-nums on stats/percent/numeric inputs; purge button: aria-busy + label "Purge data" → "Purging…" while pending; warning notice gets role=note. (commit `2c469d3`)
**ai-settings sentence-case + aria-busy (314):** ✅ complete — card title "AI Settings" → "AI settings"; field labels "AI Response Style"/"Default AI Agent"/"Remember Context" → sentence-case (acronym preserved); toast title "Settings Updated" → "Settings updated"; Save CTA "Save Settings" → "Save settings"; Sparkles/Save/Loader2 aria-hidden; save button gets type=button + aria-busy + label flips "Save settings" → "Saving…" while pending. (commit `cf39d8a`)
**dashboard-settings ul/li + per-widget aria-labels (315):** ✅ complete — DialogTitle "Customize Dashboard" → "Customize dashboard"; widget rows `<div>` → `<ul aria-label>/<li>` with list-none/p-0/m-0 semantic; Move-up/down generic aria-label "Move widget up/down" → dynamic `Move ${widgetLabel} up/down`; Switch gains aria-label `Show ${widgetLabel} widget`; 5 lucide icons aria-hidden (Settings ×2, GripVertical, ChevronUp/Down, RotateCcw); 5 buttons gain type=button; "Saving..." → "Saving…" + Save button aria-busy. (commit `7fe5a4c`)
**provider-settings sentence-case + status aria-label (316):** ✅ complete — card titles "AI Providers"/"SMS Providers"/"Direct Mail Providers" → sentence-case; section/badge/CostDisplay labels "Automatic Routing"/"Cost per Piece"/"Simple Tasks"/"Complex Tasks"/"PCM Letter/Postcard"/"Lob Letter/Postcard" → sentence-case; StatusBadge gains dynamic aria-label `${label}: active|inactive` (SR hears connection state); 7 lucide icons aria-hidden (CheckCircle2/XCircle/Cpu/MessageSquare/Mail/DollarSign×4 in conditional renders); CostDisplay numeric cell gets tabular-nums. (commit `1ce90ff`)
**lcs-benchmark-popover dl/dt/dd + progressbar (317):** ✅ complete — "Industry Benchmark" → "Industry benchmark" + stable id for aria-labelledby; score table `<div>` stacks → semantic `<dl>/<dt>/<dd>` (description list is the right primitive for label→value pairs) with tabular-nums; visual percentile bar promoted to role=progressbar w/ aria-labelledby + aria-valuenow/min/max (SR hears percentile rank, not just trend text); 3 trend icons aria-hidden (TrendingUp/Down/Minus — text already conveys direction); percentile number wrapped in tabular-nums span. (commit `80c6f53`)
**provider-settings-cards credential + per-card aria-labels (318):** ✅ complete — statusConfig "Not Configured" → "Not configured"; "API Key"/"Test Connection"/"Monthly Usage" → sentence-case; bare `<label>` → shadcn `<Label>`; API key input gains full secret-hardening stack (autoCapitalize/autoCorrect/spellCheck off alongside autoComplete=off + type=password); test-result notice gets role={success?status:alert} + aria-live; Test/Save buttons get type=button + aria-busy + label flips "Test connection"→"Testing…", "Save"→"Saving…"; 9 lucide icons aria-hidden; status pill gets role=status; Details disclosure gains aria-expanded + dynamic `Collapse|Expand ${displayName} details` + focus-visible ring; Configure button gets type=button + aria-label `Configure ${displayName}`; placeholder ellipsis fix. (commit `a5e51e9`)
**shared-ui trio polish (319):** ✅ complete — three high-leverage shared primitives: copy-button gets icon aria-hidden + sr-only role=status success announcement; export-button gets type=button + aria-busy + label flips "Export X" → "Exporting…" + Download/Loader2 aria-hidden; page-header outer wrapper `<div>` → semantic `<header>` (landmark) + icon slot wrapped in `<span aria-hidden>` so consumer icons never duplicate the title in SR output. (commit `2417bef`)
**conversion-funnel chart a11y (320 — milestone):** ✅ complete — canonical visualization used across pipeline header / analytics / dashboards: outer container gains role=img + computed aria-label summary "Conversion funnel: ${stage} ${count}, …" so SR users hear the data instead of empty bar divs; visual bar row aria-hidden=true (presentational); ChevronRight separator aria-hidden; counts/percents wrapped in tabular-nums; legend `<div>` rows → `<ul aria-label="Funnel stages">/<li>` semantic with color swatch as `<span aria-hidden>`. (commit `a321cfc`)
**deal-journey sentence-case + step-of-N aria-label (321):** ✅ complete — canonical 6-stage progress viz on every deal card/drawer: stage labels "Offer Sent"/"In Escrow" → sentence-case (propagates into aria-label); aria-label enriched "Deal stage: ${label}" → "Deal stage: ${label} (step ${i+1} of ${N})" matching keyboard-shortcuts step-of-M pattern; visual stage row + cancelled banner marked aria-hidden (role=img wrapper's aria-label is the SR target — every inner icon/label is presentational). (commit `dd10fff`)
**animated-counter + filters-chip (322):** ✅ complete — animated-counter: SRs would announce every in-flight tween frame, spamming the user; visible text wrapped in `<span aria-hidden>` + outer span gets aria-label={formatter(value)} so SR hears only the destination, never intermediate tween values. filters-chip: parenthetical count "(3)" → dynamic aria-label "Filters, 3 active"; Filter icon aria-hidden; count wrapped in tabular-nums span; type=button. (commit `72d7162`)
**TasksDueWidget asChild Link sweep (323):** ✅ complete — dashboard founder-home widget: CardTitle "Tasks Due"/"Due Today"/"View All Tasks" → sentence-case throughout; invalid Wouter `<Link><Button>` nesting (button inside anchor) → canonical `<Button asChild><Link>...</Link></Button>` in 3 places; per-row Complete-task aria-label generic → dynamic `Complete task: ${task.title}` + aria-busy; View-linked aria-label generic → dynamic `View linked ${task.entityType}`; TaskItem `<div>` → `<li>`; section task lists `<div>` → `<ul>` w/ list-none/p-0/m-0; section wrappers → `<section aria-labelledby>` with stable IDs; failed-to-load notice gets role=alert; 6 lucide icons aria-hidden; counts get tabular-nums. (commit `96df4c5`)
**NextBestActions asChild Link + ul/li (324):** ✅ complete — founder-home dashboard widget: CardTitle "Next Best Actions" → "Next best actions" (3 branches); invalid Wouter `<Link><Button>` → `<Button asChild><Link>`; action-button aria-label generic → dynamic `${action.actionLabel}: ${action.title}` (SR hears which entity the action applies to); action-card `<div>` → `<li>` inside `<ul aria-label="Recommended actions">`; 6 lucide icons aria-hidden (Phone/Mail/FileText/Calendar/Clock/Lightbulb); priority badge gains aria-label "${priority} priority"; header count badge tabular-nums. (commit `80864d3`)
**asChild link sweep #1 — 4 surfaces (325):** ✅ complete — fixes invalid Wouter `<Link><Button>` nesting (button-inside-anchor is invalid HTML + two-interactive-nodes-at-same-level breaks SR nav) across activity-feed.tsx (View), onboarding-checklist.tsx (Go to step + dynamic aria-label `Go to ${step.label}`), playbooks/PlaybookSteps.tsx (action button + sibling complete-step gains type=button), freedom-progress-card.tsx (Create first note + Set target — sentence-case CTAs); 4 lucide icons aria-hidden across the surfaces. (commit `b0588be`) — sweep continues; ~21 files still match the pattern.
**asChild link sweep #2 — 4 surfaces (326):** ✅ complete — notification-center.tsx (View all activity footer), seller-intent-panel.tsx (per-row open lead with aria-label on Link so the <a> carries SR target), user-morning-briefing.tsx (priority CTA + "Today's Priority" → "Today's priority"), provider-readiness-banner.tsx (Setup compact + Configure integrations sentence-case in 2 banners); 5 lucide icons aria-hidden. (commit `78864ef`)
**asChild link sweep #3 — 4 surfaces (327):** ✅ complete — activity-content.tsx (per-row View details + dynamic aria-label `View ${entityType} details`), deal-feed/daily-deal-feed.tsx (Configure target counties + View all N — sentence-case + tabular-nums count), forgot-password.tsx (Back to sign in + Return to sign in), reset-password.tsx (Request new link). 3 lucide icons aria-hidden. (commit `9b71231`)
**asChild link sweep #4 — pages/dashboard + night-cap (328):** ✅ complete — pages/dashboard.tsx (View all → deal-feed, Go to leads, Go to campaigns), pages/night-cap.tsx (Dashboard header + View all leads); aria-labels migrated onto the rendered `<a>` so SR targets land naturally. (commit `fbd332b`)
**asChild link sweep #5 — today.tsx (329):** ✅ complete — founder home (most-trafficked page) — all 12 invalid `<Link><Button>` nestings flipped: Get started, Evening review, All tasks, per-row alert link, View all (pax insights), Follow up (stale-lead row), View deal (expiring-offer row), All leads (briefing header), Manage goals, View pipeline (AI action queue), View finance (notes header). All inline icons already aria-hidden in prior slices. (commit `38da699`)
**asChild link sweep #6 — sovereign-dashboard.tsx (330 — milestone):** ✅ complete — all 10 invalid `<Link><Button>` flipped: 4 tab footers (Board of directors, Performance review, Full event log, Full job dashboard) + 6 module grid items (Board of directors, Agent performance, Memory browser, Event log, Job health, Founder HQ); all inline icons already aria-hidden. (commit `f6c5dca`)
**asChild link sweep #7 — 7 pages, 10 violations (331):** ✅ complete — deals.tsx (3: Create package + empty-state + per-package View), ops-dashboard.tsx (1: View all + variable aria-label moves onto <Link>), finance.tsx (1: Stripe empty state), market-data.tsx (1: Start free auth CTA), terms.tsx (1: Back to AcreOS), privacy.tsx (1: Back to AcreOS), certification-requirements.tsx (2: View leaderboard + per-tier Start working toward). (commit `c554bc9`) — entire `<Link><Button>` sweep complete; zero remaining violations across client/src.
**ellipsis sweep #1 — 7 surfaces (332):** ✅ complete — three-dot "..." (SR "dot dot dot") → typographic ellipsis "…" (SR "ellipsis") across Input/Textarea/Select placeholders: tasks.tsx (Task title + description), leads.tsx (Search leads ×2, Choose a property, Enter offer amount + Label sentence-case), automation.tsx (Select action), properties.tsx (description), counties.tsx (Notes), conversation-tray.tsx (Type a message), property-analysis-chat.tsx (Ask about this property). (commit `8f7ebb2`)
**ellipsis sweep #2 — 14 surfaces (333):** ✅ complete — completes the sweep; zero remaining placeholder "..." in client/src. command-center (4 + sentence-case Property ID), HelpPanel (2), support-content, field-scout (2), MobileCommandDrawer (+ "Quick Actions" → "Quick actions" + Search icon aria-hidden), CompanyChronicle (+ Search icon aria-hidden), WarRoom, InitiativeBoard, inspection-checklist, photo-gallery, due-diligence-panel, document-generator (+ "Additional Terms" → sentence-case), research-summary-panel, investor-directory (Bio + self-attestation), founder-dashboard internal notes. (commit `f2ecf31`)
**DecisionsInbox ul/li + aria-expanded (334):** ✅ complete — founder-home triage widget: CardTitle "Decisions Inbox" → "Decisions inbox" (both branches); pending badge gets tabular-nums + aria-label `${pending} pending`; items rendered loose → `<ul aria-label="Pending decisions">` with ItemCard `<div>` → `<li>`; toggle-context button gains type=button + aria-expanded + dynamic aria-label "Show/Hide context" + focus-visible ring; agent avatar emoji aria-hidden; 4 lucide icons aria-hidden; Approve/Reject/Defer buttons gain type=button + aria-busy. (commit `469ee82`)
**ThePulse section landmark + ul/li (335):** ✅ complete — founder-home traffic-light grid: light titles "Revenue Health"/"System Health"/"Sophie Health"/"Churn Risk" → sentence-case; outer `<div>` → `<section aria-labelledby="the-pulse-heading">` (landmark); all-clear banner gets role=status + aria-live; light grid `<div>` → `<ul aria-label="Health indicators">/<li>`; each light Card gains computed aria-label `${title}: ${light.label}`; color dot span aria-hidden; removed unused Badge/AlertCircle imports. (commit `ac44a2f`)
**PredictiveInsights ul/li + role=img sparkline (336):** ✅ complete — founder-home forecast widget: CardTitle "Predictive Insights" → "Predictive insights" (3 branches); TrendingUp aria-hidden; `predictions.map` → `<ul aria-label="Predictions">/<li>`; per-prediction sparkline `<div>` gains role=img + computed aria-label `${title} trend: projected ${value} ${timeframe}` so SR hears chart data instead of empty svg; projected value tabular-nums. (commit `c51b5d7`)
**AnomalyAlerts ul/li + dynamic aria-label (337):** ✅ complete — founder-home weekly-deviation widget: CardTitle "Anomaly Alerts" → "Anomaly alerts" (3 branches); 4 lucide icons aria-hidden (AlertTriangle ×3 + TrendingUp/Down/Minus); anomalies loose-rendered `<a>` → `<ul aria-label="Detected anomalies">/<li>/<a>` w/ focus-visible ring; per-anomaly `<a>` gains computed aria-label `${message} — ${pct}% (${current} vs ${previous} last week)` (was fragmented across spans + badge); header count badge tabular-nums + `${count} detected`; numeric inner lines tabular-nums. (commit `9f4890f`)
**JobQueueHealth ul/li + per-job aria-label (338):** ✅ complete — founder-home background-job dashboard: CardTitle "Job Queue Health" → "Job queue health"; loading skeleton wrapper gets role=status + aria-busy + aria-label; header summary tabular-nums; refresh button type=button + aria-label + RefreshCw aria-hidden; job grid `<div>` → `<ul aria-label="Background jobs">/<li>`; per-job aria-label `${name}: ${status}${failures > 0 ? ", N consecutive failures" : ""}`; status color dot aria-hidden; numeric lines tabular-nums; "OVERDUE" → "Overdue" (sentence-case); "N fail(s)" → conditional plural; Restart button type=button + aria-busy + dynamic aria-label `Restart ${displayName}`; removed unused overallStatus var. (commit `654eb08`)
**BusinessIntelligence section + MetricCard aria-label (339):** ✅ complete — founder-home BI panel (ARR/LTV:CAC/Churn/NRR/Cohort/Customer health): titles + BAND_LABELS sentence-case; outer `<div>` → `<section aria-labelledby="bi-heading">` (landmark); loading skeleton role=status + aria-busy; MetricCard gains computed aria-label `${title}: ${value}, ${trendLabel} (${sub})` (was 3-4 silent fragments); TrendIcon aria-hidden (info flows via aria-label); metric value tabular-nums; Customer health rows `<div>` → `<ul aria-labelledby>/<li>` with per-row aria-label `${label}: ${count} (${pct}%)` + color dot aria-hidden + numeric cells tabular-nums. (commit `1c05a38`)
**GrowthEngine ul/li + per-tier aria-label (340 — milestone):** ✅ complete — founder-home tier distribution + conversion funnel + expansion signals: CardTitle "Growth Engine" → "Growth engine"; loading skeleton role=status + aria-busy; header net-expansion badge tabular-nums + computed aria-label; tier rows `<div>` → `<ul aria-labelledby>/<li>` with per-tier aria-label `${label}: ${count} (${pct}%)` + bar aria-hidden + tabular-nums; conversion funnel cards gain computed aria-labels (free accounts/converted/conversion rate) + connectors aria-hidden; expansion-signals duo `<div>` → `<ul aria-label>/<li>` with per-side aria-label; growth opportunities `<p>` rendering → `<ul aria-labelledby>/<li>` with arrow aria-hidden; 4 lucide icons aria-hidden; removed unused payingTiers var. (commit `a267544`)
**ChurnIntelligence dl/dt/dd + ul/li + per-org aria-label (341):** ✅ complete — founder-home churn watchlist: CardTitle "Churn Intelligence" → "Churn intelligence"; loading skeleton role=status + aria-busy; status badge `Status: ${status}` aria-label + dot aria-hidden; 3-up rate-vs-benchmark grid `<div>` → semantic `<dl>/<dt>/<dd>`; visual comparison bars block aria-hidden=true (dl carries SR); at-risk rows `<div>` → `<ul aria-labelledby>/<li>` with per-org aria-label `${name}, ${tier} tier, ${idleText}, ${signal} risk` (was 4-5 fragmented spans); signal dot + Clock + AlertTriangle + CheckCircle2 aria-hidden; counts/idle text tabular-nums; empty state gets role=status; recommendations `<p>` → `<ul/li>` with arrow aria-hidden. (commit `c4595f8`)
**MRRTrajectory role=img chart + dl/dt/dd + ul/li legend (342):** ✅ complete — founder-home revenue area chart: CardTitle "Revenue Trajectory" → "Revenue trajectory"; loading skeleton role=status + aria-busy; BarChart3 + MomIcon aria-hidden; MoM + ARR badges gain tabular-nums + computed aria-labels; sparkline stats row `<div>` → `<dl>/<dt>/<dd>` w/ tabular-nums + dividers aria-hidden; recharts wrapped in `<div role="img" aria-label>` with computed chart summary; legend `<div>` rows → `<ul aria-label="Chart legend">/<li>` w/ swatches aria-hidden. (commit `df947da`)
**PlatformPassiveScore role=progressbar + ul/li + role=alert (343):** ✅ complete — founder-home automation health (largest dashboard surface): CardTitle "Platform Passive Score" → "Platform passive score"; loading skeleton role=status + aria-busy; ScoreRing custom SVG donut wrapped with role=progressbar + aria-label `Passive score: N out of 100` + aria-valuenow/min/max + inner SVG aria-hidden; header acts/week badge tabular-nums + aria-label; missing creds pills `<span>` → `<ul aria-labelledby>/<li>`; score breakdown bars `<div>` → `<ul/li>` with per-row aria-label `${label}: ${score} out of 100`; automation activity bars `<div>` → `<ul/li>` with per-row aria-label `${name}: ${actions/idle}, status ${status}` + status icons aria-hidden + bar aria-hidden + counts tabular-nums; operational health notice gets role=alert + count spans tabular-nums. (commit `370162e`) — **dashboard subtree complete** (12/12 widgets refined: TasksDueWidget→PlatformPassiveScore).
**FocusCard aria-busy + regen aria-label (344):** ✅ complete — Sovereign Protocol v7 daily-priority widget on founder home: loading Skeleton role=status + aria-busy + aria-label; empty-state Generate button gains type=button + aria-busy + label flips "Generate" → "Generating…"; filled-state regenerate icon button gains type=button + aria-busy + dynamic aria-label "Regenerate focus card"/"Regenerating focus card"; Crosshair + RefreshCw aria-hidden. (commit `d9b6e85`)
**MorningBriefing role=progressbar + ul/li (345):** ✅ complete — Sovereign Protocol v3 daily summary: HealthRing custom SVG donut wrapped with role=progressbar + aria-label + aria-valuenow/min/max + inner SVG aria-hidden + score tabular-nums; loading skeleton role=status + aria-busy; AgentUpdateCard motion.div → motion.li with avatar aria-hidden + active-pulse dot role=status + aria-label="Active"; pending decisions badge tabular-nums + computed aria-label; all-clear notice role=status; trust updates motion.div → motion.ul/li with avatars aria-hidden; agent updates motion.div → motion.ul aria-label; timestamp wrapped in semantic `<time dateTime>` + tabular-nums; Clock aria-hidden. (commit `33e0954`)
**AbsenceMode role=alert + ul/li + sentence-case (346):** ✅ complete — Sovereign Protocol v6 away-status widget: titles "Away Mode Active"/"Absence Mode"/"Emergency Breaks"/"Batched for Your Return"/"Return Briefing" → sentence-case + "I'm Back" → "I'm back"; loading skeleton role=status + aria-busy; both action buttons get type=button + aria-busy + label flips ("Returning…", "Activating…"); status row `<div>` → `<ul/li>` with per-row computed aria-label + tabular-nums; emergency breaks gets role=alert + `<ul/li>`; batched items + return briefings promoted to `<ul/section aria-labelledby>` semantic; duration Select gains aria-label; 9 lucide icons aria-hidden. (commit `3efb426`)
**AgentGrowth ul/li + per-goal/skill aria-label (347):** ✅ complete — Sovereign Protocol v7 agent self-improvement widget: titles "Agent Growth"/"Skill Requests"/"Generate Plans" → sentence-case; loading skeleton role=status + aria-busy; GoalProgress `<div>` → `<li>` with computed aria-label `${description}: achieved` or `${description}: ${pct}% — current N, target M`; tabular-nums on baseline/current/target; PlanCard `<div>` → `<li>` w/ avatar aria-hidden + status badge capitalize + aria-label; goals + plans + skill-requests promoted to `<ul aria-label{,edby}>/<li>`; Grant/Deny buttons gain type=button + aria-busy + dynamic aria-label `Grant ${skill} to ${role}` / `Deny ${skill} for ${role}`; Generate-plans button gets aria-busy + label flip + tabular-nums count badge. (commit `b8b8489`)
**SwipeDecisionCard role=status flash + aria-expanded (348):** ✅ complete — Sovereign Protocol v3 Tinder-style decision-approval card: swipe-direction Approve/Reject visual overlays aria-hidden (purely visual cues, sibling buttons carry SR); success/reject flash overlay gains role=status + aria-live + dynamic aria-label "Approved"/"Rejected" (was silent visual); avatar role=img → aria-hidden (sibling text label is the SR target); estimated impact wrapped in tabular-nums; details disclosure button gets type=button + aria-expanded + focus-visible ring; Approve/Reject/Defer buttons type=button + aria-busy + icon-only Reject/Defer get aria-label "Reject"/"Defer 24 hours"; swipe hint aria-hidden (touch-only affordance). (commit `3a47523`)
**AgentTeamChat role=radiogroup selector (349):** ✅ complete — Sovereign Protocol v3 iMessage-style team chat: AgentSelector `<div>` → `<div role="radiogroup" aria-label="Route to agent">`; each agent button gains role=radio + aria-checked + aria-label={AGENT_ROLES[agent]} (SR hears role even when only avatar visible on mobile) + type=button + focus-visible ring; avatar emojis aria-hidden across selector + message bubbles + empty state; Send button gains aria-busy + Send icon aria-hidden. (commit `0ef0b67`)
**CustomerHealthPanel role=img + ul/li (350 — milestone):** ✅ complete — Sovereign Protocol v6 customer-health watchlist: loading wrapper role=status + aria-busy; "Loading..." → "Loading…"; Heart + AlertTriangle + TrendIcons aria-hidden; health distribution bar `<div>` → role=img + computed aria-label `Distribution: N healthy, M at risk, K critical` (segment titles weren't SR-accessible); customer rows `<div>` → `<ul/li>` w/ per-row computed aria-label `${orgName}: health N, ${trend}, ${plan}, ${loginText}`; numerics tabular-nums. (commit `4caeda8`)
**AgentDebatePanel ul/li + section landmarks + aria-busy (351):** ✅ complete — Sovereign Protocol v8 structured agent disagreement: "FOR"/"AGAINST"/"REBUTTALS"/"CEO Decision:" → sentence-case; status badge gets aria-label + Loader2 aria-hidden; for/against columns + rebuttals → `<section aria-labelledby>` + `<ul aria-labelledby>/<li>`; avatar emojis aria-hidden; vote summary panel + vote-detail badges promoted to `<ul aria-label>/<li>` with computed per-vote aria-label `${role}: ${vote}`; debate timestamp wrapped in `<time dateTime>` + tabular-nums; Approve/Reject/Defer + Debate-initiate buttons gain type=button + aria-busy + label flip; CEO decision badge gets capitalize; DebateCard `<div>` → `<li>`; debate list promoted to `<ul aria-label="Debates">`; pending count badge tabular-nums + aria-label. (commit `a5d84bd`)
**DecisionAutopilot section landmarks + ul/li + per-pattern aria-label (352):** ✅ complete — Sovereign Protocol v7 pattern-learning widget: titles "Decision Autopilot"/"Ready for Autopilot"/"On Autopilot"/"Learning Your Patterns" → sentence-case; loading skeleton role=status + aria-busy; shadow accuracy line gains aria-label + tabular-nums; stats bar `<div>` → `<ul aria-label>/<li>` with per-stat computed aria-label; 3 pattern sections promoted to `<section aria-labelledby>/<ul/li>`; PatternRow `<div>` → `<li>` w/ computed aria-label combining role + description + decisions/approved/confidence + autopilot state; Switch gets dynamic aria-label; Progress bar gets aria-label; all numerics tabular-nums. (commit `fe47232`)
**PerformanceReviews dl/dt/dd + section landmarks + ul/li (353):** ✅ complete — Sovereign Protocol v6 weekly evaluations: titles "Performance Reviews"/"Generate Reviews"/"To Improve"/"Peer Feedback" → sentence-case; loading skeleton role=status + aria-busy; generate-reviews button type=button + aria-busy + label flip; ReviewCard `<div>` → `<li aria-labelledby>` w/ stable heading id + parent `<ul aria-label>`; period dates wrapped in `<time dateTime>` + tabular-nums; grade chip aria-label; metrics 4-up grid → semantic `<dl>/<dt>/<dd>` with trust score computed aria-label; strengths/improvements/peer feedback → `<section aria-labelledby>/<ul/li>` semantic; peer-feedback row prepends agent role to "" (was orphan emoji + raw quote); 6 lucide icons aria-hidden; numerics tabular-nums. (commit `2788943`)
**ActivityTimeline section landmarks + ul/li + aria-expanded (354):** ✅ complete — Sovereign Protocol v4 chronological action trace: loading skeleton role=status + aria-busy; empty-state Clock aria-hidden; time blocks `<div>` → `<section aria-labelledby>` + entry `<div>` → `<ul aria-labelledby>/<li>` semantic; expand button gains type=button + aria-expanded + computed aria-label `${agentName}: ${action}, ${outcomeText} at ${time}` (was 4-5 fragmented spans + 3 silent icons); agent dot aria-hidden; timestamp wrapped in `<time dateTime>` + tabular-nums; 5 lucide icons aria-hidden (CheckCircle/XCircle/Clock outcome + ChevronUp/Down + Undo2); Undo button type=button + aria-busy + dynamic aria-label + label flip + focus-visible ring. (commit `d4a706d`)
**DelegationManager section landmark + ul/li + per-row aria-label (355):** ✅ complete — Sovereign Protocol v6 active temporary-delegations: "Active Delegations" → "Active delegations"; outer `<div>` → `<section aria-labelledby>` w/ heading promoted to `<h3>` + stable id; rows loose-mapped `<div>` → `<ul aria-labelledby>/<li>` w/ per-row aria-label `${name} elevated to level N, ${expiryText}, reason: ${reason}`; Shield/Clock/X aria-hidden; level + expiry tabular-nums; Revoke button type=button + aria-busy + dynamic aria-label + focus-visible ring (was inaccessible `title` attribute only). (commit `0291932`)
**SynergyMap ul/li + per-pair aria-label (356):** ✅ complete — Sovereign Protocol v8 agent-pair collaboration scores: "Agent Synergy Map" → "Agent synergy map"; loading skeleton role=status + aria-busy; SynergyRow `<div>` → `<li>` w/ computed aria-label `${roleA} + ${roleB}: synergy N%, M collaborations${bestAt}${recommendation}` (was 4-5 fragmented spans); avatar pair wrapper aria-hidden; Network/ArrowUpRight aria-hidden; synergy progress bar gets aria-label; score + collab count tabular-nums; rendering loose `.map()` → `<ul aria-label="Agent pair synergy">`. (commit `2fced19`)
**InstitutionalMemory section landmarks + ul/li (357):** ✅ complete — Sovereign Protocol v7 compound knowledge: titles "Institutional Memory"/"Discover Patterns"/"Learned Patterns"/"Signal Correlations" → sentence-case; loading skeleton role=status + aria-busy; discover button gains type=button + aria-busy + label flip; PatternCard `<div>` → `<li>` w/ computed aria-label `Pattern ${name}: N% across M observations`; CorrelationCard `<div>` → `<li>` w/ computed aria-label combining signals + outcome + strength + observations; conditions badges → `<ul/li>`; contributing agents promoted to `<ul/li aria-label={agent}>` (was inaccessible `title`); pattern list + correlation list outer wrappers → `<section aria-labelledby>/<ul aria-labelledby>` semantic; counts/percents tabular-nums; 4 lucide icons aria-hidden. (commit `6c45351`)
**FounderTwin ul/li + per-draft aria-label (358):** ✅ complete — Sovereign Protocol v7 voice-trained draft generator: DRAFT_TYPES labels "Investor Update"/"Board Report"/"Customer Response"/"Team Announcement" → sentence-case + "Your Digital Twin" → "Your digital twin"; loading skeleton role=status + aria-busy; DraftCard `<div>` → `<li>` w/ status badge capitalize + aria-label; timestamp `<time dateTime>` + tabular-nums; Edit textarea/topic Input/draft-type Select gain aria-label; Edit/Save/Approve/Copy buttons type=button + aria-busy + dynamic aria-label `${verb} ${draft.title}` + label flips while pending; Generate-draft button aria-busy + label flip; 5 icons aria-hidden; drafts list `<map>` → `<ul aria-label="Drafts">`. (commit `9e42c9b`)
**TrendCards + OutcomeFeedback paired sweep (359):** ✅ complete — TrendCards: loading skeleton role=status + aria-busy; trend cards `<div>` → `<ul aria-label="This week vs last week trends">/<li>` w/ computed per-card aria-label `${label}: N, ${direction} +X% vs M previous`; MiniSparkline svg aria-hidden; current/delta tabular-nums. OutcomeFeedback: "Agent Performance" → sentence-case; outer `<div>` → `<section aria-labelledby>` w/ heading `<h3>`; 3-up outcome grid `<div>` → `<ul aria-label="Outcome breakdown">/<li>` w/ per-cell aria-label; 4 lucide icons aria-hidden; counts/percent tabular-nums. (commit `e2b36fc`)
**FounderWellbeingCard role=progressbar + ul/li (360 — milestone):** ✅ complete — Sovereign Protocol v8 founder energy widget: "Founder Wellbeing" → "Founder wellbeing"; loading skeleton role=status + aria-busy; EnergyBar visual gains role=progressbar + aria-label + aria-valuenow/min/max; empty-state + header buttons type=button + aria-busy + dynamic aria-label + label flip "Assess" → "Assessing…"; metrics row `<div>` → `<ul aria-label>/<li>` w/ per-metric computed aria-label; insights `<div>` rows → `<ul/li>` w/ role=alert for warnings, role=status for celebrations/milestones; 4 icons aria-hidden + tabular-nums. (commit `06990ab`)
**DecisionQuality aria-expanded button + dl + section landmarks (361):** ✅ complete — founder-home expandable widget: titles "Decision Quality"/"Approval Rate"/"Positive Outcomes"/"Per-Agent Accuracy"/"Recent Outcomes" → sentence-case; loading skeleton role=status + aria-busy; clickable CardHeader `<div>` w/ onClick → `<button type=button aria-expanded aria-label>` w/ focus-visible ring (was inaccessible — keyboard couldn't activate); 3-up summary stats `<div>` → semantic `<dl>/<dt>/<dd>`; per-agent accuracy + recent outcomes sections promoted to `<section aria-labelledby>/<ul/li>` w/ per-row computed aria-label; 4 icons aria-hidden + all numerics tabular-nums. (commit `1655bc2`)
**WorkflowMonitor section landmarks + ul/ol/li (362):** ✅ complete — Sovereign Protocol v6 pipeline real-time monitor: titles "Agent Pipelines"/"Available Pipelines"/"Recent Runs" → sentence-case; loading skeleton role=status + aria-busy; StepPill gains computed aria-label `${role}: ${action}, status ${status}${duration}` (was fragmented spans + silent icon); avatar + status icons aria-hidden; RunCard `<div>` → `<li>` w/ status badge capitalize + aria-label, timestamp `<time dateTime>` + tabular-nums; step rows promoted to `<ol aria-label>` (ordered — sequence matters); arrow connector aria-hidden; available pipelines + recent runs → `<section aria-labelledby>/<ul/li>` semantic; Run button gets type=button + aria-busy + dynamic aria-label; active runs count badge tabular-nums + aria-label. (commit `eccac59`)
**PlaybookManager section landmarks + ul/ol/li + sr-only step role (363):** ✅ complete — Sovereign Protocol v6 SOP review/approval: "Needs Your Approval"/"Active Playbooks"/"Run Now" → sentence-case; loading skeleton role=status + aria-busy; PlaybookCard `<div>` → `<li>`; status badges aria-label; owner avatar aria-hidden; steps `<div>` → `<ol aria-label>` w/ step number aria-hidden + sr-only `${role}: ` prepended to description (was orphan emoji); stats row `<div>` → `<ul/li>` w/ counts tabular-nums + last-executed `<time dateTime>`; toggle Switch + Approve/Run-now buttons gain dynamic aria-labels + aria-busy + label flips; pending + approved → `<section aria-labelledby>/<ul/li>` semantic; pending count badge tabular-nums + aria-label; 3 icons aria-hidden. (commit `524fde0`)
**ScenarioEngine ul/li + per-outcome aria-label (364):** ✅ complete — Sovereign Protocol v7 "what if?" simulator: OUTCOME_STYLES "Best Case"/"Most Likely"/"Worst Case" → sentence-case; loading skeleton role=status + aria-busy; ScenarioCard `<div>` → `<li>` w/ status badge capitalize + aria-label; timestamp `<time dateTime>` + tabular-nums; agent analyses `<div>` rows → `<ul/li>` w/ per-row computed aria-label `${role} (${perspective}, ${confidence}): ${analysis}` (was 4 fragmented spans + silent confidence badge); outcome scenarios → `<ul/li>` w/ per-cell aria-label `${label}: ${desc}, N% likely`; recommendation `<div>` → `<section aria-labelledby>`; hypothesis Input gains aria-label + autoCapitalize; Simulate button type=button + aria-busy + label flip; 5 icons aria-hidden; numerics tabular-nums. (commit `91a7ae2`)
**StrategicCompass button trigger + role=radiogroup (365):** ✅ complete — Sovereign Protocol v8 living-strategy widget: "Strategic Compass"/"{Mode} Mode" → sentence-case; loading skeleton role=status + aria-busy; mode badge aria-label; clickable north-star `<div>` w/ onClick → `<button aria-label="Edit north star statement">` w/ focus-visible ring (was keyboard-inaccessible); edit form Input gains aria-label + autoCapitalize + autoFocus, Save/Cancel buttons type=button + aria-busy + label flip; priorities `<div>` flex → `<ul/li>` w/ per-priority computed aria-label + tabular-nums rank + star repetition aria-hidden; mode switcher → role=radiogroup + each Button role=radio + aria-checked + aria-busy. (commit `212cbc9`)
**CompanyChronicle aria-expanded button + ul/li + section landmarks (366):** ✅ complete — Sovereign Protocol v8 AI-generated narrative history: "Company Chronicle"/"Key Learnings" → sentence-case; loading skeleton role=status + aria-busy; ChronicleCard `<div>` → `<li>` w/ clickable header `<div>` → `<button aria-expanded aria-controls aria-label>`; top-agent emoji aria-hidden + aria-label outer span; counts tabular-nums; periodType badge capitalize; disclosure indicator aria-hidden; highlights + key learnings → `<section aria-labelledby>/<ul/li>` w/ stable ids + sr-only highlight type prefix; Generate Week/Month buttons type=button + aria-busy + dynamic aria-label; search Input aria-label; chronicle list `<map>` → `<ul aria-label>` semantic; 5 icons aria-hidden. (commit `d2649f0`)
**InitiativeBoard ul/li + section landmarks + radiogroup filter (367):** ✅ complete — Sovereign Protocol v6 strategic proposal/approval: "Agent Initiatives" → sentence-case; loading skeleton role=status + aria-busy; InitiativeCard `<div>` → `<li>` w/ status badge capitalize + aria-label; created date `<time dateTime>` + tabular-nums; evidence → `<section aria-labelledby>/<ul/li>` w/ type badge capitalize; projected impact panel gains computed aria-label; required agents promoted to `<ul/li aria-label>` (was inaccessible `title`); Textarea aria-label; Approve/Shelve/Reject buttons type=button + aria-busy + dynamic per-initiative aria-label; + Note disclosure aria-expanded + aria-controls; pending count tabular-nums + aria-label; filter switcher → role=radiogroup w/ Buttons role=radio + aria-checked; 4 icons aria-hidden. (commit `171e98c`)
**WarRoom role=log + ul/li + aria-busy (368):** ✅ complete — Sovereign Protocol v6 critical-event Slack-like thread: titles "War Rooms"/MESSAGE_TYPE_LABEL "Action Plan"/"Action Taken"/"CEO Directive" → sentence-case; loading skeletons role=status + aria-busy (top-level + inner messages); WarRoomThread `<div>` → `<li>`; participants/active count badges tabular-nums + aria-label; Resolve button type=button + aria-busy + dynamic aria-label + label flip; lead-agent emoji aria-hidden + role label SR target; created timestamp `<time dateTime>` + tabular-nums; messages container gains role=log + aria-live="polite" + aria-label (canonical chat semantic); MessageBubble `<div>` → `<li>` w/ avatar aria-hidden + `<time dateTime>` + tabular-nums; CEO directive Input + Send button gain aria-label + aria-busy; resolution notice role=status; war rooms `<map>` → `<ul aria-label>`; ellipsis fix; 4 icons aria-hidden. (commit `bef1ea9`)
**mobile pair: PullToRefresh + CommandDrawer (369):** ✅ complete — PullToRefresh: indicator gains role=status + aria-live + dynamic aria-label per state ("Pull to refresh"/"Release to refresh"/"Refreshing…") so SR users hear gesture state instead of silent visual ring; icons aria-hidden. CommandDrawer: "Quick Actions" → "Quick actions"; Quick actions + More sections `<div>` → `<section aria-labelledby>/<ul aria-labelledby>/<li>` semantic; each Link gets aria-current="page" when active + focus-visible ring; quick-action icon-circle wrapper aria-hidden; more-section icons aria-hidden; empty-search-results gains role=status + aria-live. (commit `69e9613`)
**empty-states quintet type=button (370 — milestone):** ✅ complete — empty-states subtree (Leads/Deals/Properties/Campaigns/Tasks) was already well-built (decoration wrappers + inline icons aria-hidden + framer-motion respect); final polish: explicit type=button on every CTA Button (7 total across 5 files) so React/HTML default "submit" never bubbles up if any consumer wraps in a parent form. Defensive against future regressions. (commit `b1a0b04`)
**PlaybookCard ul/li + aria-busy + sentence-case (371):** ✅ complete — playbook template card: sentence-case "Due Diligence"/"In Progress"/"Start Playbook"/"Restart Playbook"/"Playbook Started" toast → all sentence-case; "Starting..." → "Starting…"; 5 lucide icons aria-hidden; overview row `<div>` → `<ul aria-label>/<li>` w/ per-stat aria-label; Progress bar gains aria-labelledby + aria-valuenow/min/max; Start/Continue/Restart buttons type=button + aria-busy + dynamic per-template aria-label. (commit `aa11bb5`)
**TimelineView ul/li milestones + sentence-case (372):** ✅ complete — multi-zoom milestone timeline: dealMilestone titles "Deal Created"/"Expected Close" → sentence-case + "In Progress" legend → sentence-case; 9 lucide icons aria-hidden (Calendar header + 4 status + 4 milestone-type + empty-state Calendar); Today/ZoomIn/Out buttons type=button + aria-label + zoom-level span aria-label "Current zoom: ${label}"; today vertical line marker aria-hidden; milestones loose-mapped `<div>` → `<ul aria-label="Timeline milestones">/<li>` w/ per-row computed aria-label combining title + formatted date + status + description; removed misleading cursor-pointer from pill (no onClick); legend badges → `<ul aria-labelledby>/<li>`. (commit `ddc57e8`)
**PlaybookSteps ol/li sequence + collapsible aria (373):** ✅ complete — sequenced playbook step list: step `<div>` map → `<ol aria-label="Playbook steps">/<li>` (ordered: sequence matters) w/ per-step aria-label "Step N of M: ${title} — ${status}"; "Mark Complete" → "Mark complete"; mark-complete button type=button (was missing) + aria-label; action/undo buttons gain disambiguating aria-label "${verb}: ${step title}"; CollapsibleTrigger gains aria-label "(Expand|Collapse) step: ${title}" + focus-visible ring; 7+ icons aria-hidden (status circle, vertical line, ChevronDown/Right, Clock, status-icon, inline CheckCircle2); estimated-time gains aria-label + tabular-nums; Progress bar gains full aria-valuenow/min/max; completed badge tabular-nums + aria-label. (commit `8e225e7`)
**IRRCalculator dl/ol + label htmlFor + sentence-case (374):** ✅ complete — IRR/NPV/Equity Multiple/Cash-on-Cash calculator: title + 6 input labels + 4 result-tile labels + 2 summary badges + cashflow-schedule heading → sentence-case across the board; removed unused TrendingUp import; all 6 form inputs gain id + Label htmlFor linkage (was implicit/broken — clicking Label didn't focus Input) + tabular-nums; result tiles `<div>` → `<dl/dt/dd>` semantic w/ per-metric aria-label combining name+value; summary badges → `<ul/li>` w/ aria-label per item; cashflow `<div>` map → `<section aria-labelledby>` + `<ol/li>` (Y0→Yn ordered) w/ per-year aria-label "Year N: ${amount}"; visual Y0/$ fragments aria-hidden so SR hears single announcement; Calculator + FileDown icons aria-hidden; Export button type=button + aria-label "Export IRR analysis as CSV". (commit `110aee8`)
**CalendarWidget ul/li week grid + section landmark (375):** ✅ complete — 7-day calendar grid: "This Week" → "This week"; removed 4 unused lucide imports; container `<div>` → `<section aria-labelledby>`; legend `<div>` → `<ul aria-label="Event types">/<li>` w/ color dots aria-hidden; week grid motion.div → motion.ul + each day motion.div → motion.li w/ aria-current=date when today + computed aria-label "${weekday-name} ${month} ${day} (today): N events"; per-day events `<div>` → `<ul/li>` w/ per-event aria-label "${type}: ${title} at ${time}"; "+N more" → `<li>` w/ aria-label; visual "Mon 5" fragment aria-hidden + tabular-nums on date number. (commit `0791573`)
**CommentThread ul/li + section landmark + aria-controls (376):** ✅ complete — collapsible threaded comments w/ infinite scroll: comment list `<div>` map → `<ul aria-label="Comment list">/<li>` (motion.div → motion.li); expanded thread `<div>` → `<section id aria-labelledby>` w/ stable heading id; collapsed pill + close button gain aria-controls pointing at section + aria-expanded reflecting state (was no pairing); 2-letter avatar `<div>` aria-hidden (username follows); time → semantic `<time dateTime={iso}>` + tabular-nums; delete button aria-label disambiguated "Delete your comment from {time}"; Send button type=button + aria-busy + label flip; loading skeletons aria-hidden inside parent role=status; removed unused renderContent helper. (commit `5674ad0`)
**CallLog ul/li + combined row aria-label + sentence-case (377):** ✅ complete — voice-call history w/ expandable transcripts: "Call Log"/"AI Summary" → sentence-case; removed unused Button import; calls list `<div>` map → `<ul aria-label="Call history">/<li>`; per-row button type=button + aria-controls pointing at expanded panel + computed aria-label "(Expand|Collapse) {direction} call, {date}, duration N minutes M seconds, sentiment {label}" so SR hears row state in one announcement; 5 visual fragments aria-hidden (direction wrapper, duration text, direction badge, sentiment pill, chevron + Phone icons); date → semantic `<time dateTime>` + tabular-nums; expanded detail `<div>` gains id + role=region + aria-label "Call details: {date}"; transcript `<pre>` aria-labelledby; loading transcript role=status + aria-busy; isError `<p>` role=alert; Header Phone aria-hidden. (commit `11770c5`)
**DealCalculator dl/dt/dd + sentence-case + aria-busy save (378):** ✅ complete — flip-deal ROI calculator: title + 7 form labels + 8 result-card labels + "Save Analysis"/"Saving..." → sentence-case + ellipsis canonical; 8 inline lucide icons aria-hidden (Calculator/DollarSign×5/Percent/TrendingUp/Save/RotateCcw); Calculate + Reset buttons gain explicit aria-label; Save button type=button (was missing) + aria-busy + aria-label flip; results grid `<div>` → `<dl aria-label>/<dt>/<dd>` semantic w/ per-card aria-label combining label+value ("Net profit: $42,500"); result values tabular-nums. (commit `b2e55ea`)
**CompsAnalysis ul/li everywhere + sentence-case (379):** ✅ complete — 567L comparable-property valuation panel: 9 sentence-case fixes ("Hide Filters"/"Search Radius"/"Estimated Market Value"/"Offer Suggestions"/"Property Desirability Score"/"Sale Date"/"Sale Price"/"No Comparable Properties Found"/"$/Acre" stat labels); removed unused Info import; stats grid 4-card cluster + offer tiers 3-card cluster + score breakdown all `<div>` → `<ul/li>` semantic w/ per-row aria-label combining label+value; desirability score button gains comprehensive aria-label "Property desirability score: N out of 100, grade X. (Show|Hide) breakdown"; score factor Progress bars gain aria-label + aria-valuenow/min/max (was missing); comp Table gains aria-label + sentence-case headers; all numeric cells/values tabular-nums; distance cell aria-label "N miles away"; stale-date span title promoted to aria-label so SR hears staleness warning. (commit `d1ec740`)
**ConfidenceInterval/DecisionScore/LossFrame role+aria-label (380):** ✅ complete — 3-component score viz file: 4 decision labels sentence-case ("Strong Opportunity"/"Good Opportunity"/"Evaluate Further"/"Below Threshold"); ConfidenceInterval band-bar gains role=img + tabIndex=0 + comprehensive aria-label "${label}: ${value}, 95% range ${lower} to ${upper}, ${confidenceLabel}" + focus-visible ring (was Tooltip-on-div, keyboard-unreachable); added fmt() helper deduplicating 6 unit-toggle expressions; DecisionScore wrapper gains tabIndex=0 + role=group + aria-label + focus-visible; inner progress bar gains role=progressbar + aria-valuenow/min/max (was missing); LossFrame gains role=status + aria-label combining loss+timeframe; all numbers tabular-nums; visual fragments aria-hidden where parent carries canonical label. (commit `e9f0db8`)
**ConversationTray ul/li + role=log + sentence-case (381):** ✅ complete — floating team-messaging tray (608L 4-component file): 6 sentence-case fixes ("Team Messaging"/"Team Messages"/"New Message"/"Direct Message" ×2/"Team Member"); conversation list + team-member picker `<div divide-y>` → `<ul/li>` w/ per-row aria-label combining name+context; chat messages `<div>` → `<ul role="log" aria-live="polite">/<li>` w/ per-message aria-label "{sender} at {time}: {body}"; time → semantic `<time dateTime>` + tabular-nums; 6 buttons gain type=button; Send button aria-busy + aria-label flip; message input aria-label "Message"; 10+ icons aria-hidden; loading spinners role=status + aria-busy + sr-only "Loading…"; empty-states role=status; desktop floating panel `<div>` → `<section role="dialog" aria-label>`; Avatar wrappers aria-hidden (decorative initials, name follows). (commit `6884cd1`)
**DealInbox motion.ul/li + per-row aria-label + sentence-case (382):** ✅ complete — collapsible per-lead email thread w/ reply composer + UnreadEmailBadge: "Email Thread"/"Send First Email" → sentence-case; email thread motion.div → motion.ul (aria-label) + motion.li w/ per-message aria-label "(Unread,) {direction} {from|to} {address}, {time}, subject: {subject}" combining all metadata; visual fragments aria-hidden (from/to span, direction badge, subject `<p>`, time); time → semantic `<time dateTime>` + tabular-nums; mark-all-read + reply + send-reply + send-first-email buttons gain type=button + aria-busy + aria-label; Reply/Send-reply aria-label flips; "N new" badge gains aria-label + tabular-nums; UnreadEmailBadge gains aria-label "N unread emails" (was bare digit); 7 icons aria-hidden. (commit `7bcab7f`)
**CommandPalette role=dialog + sentence-case + 16 icons aria-hidden (383):** ✅ complete — global ⌘K command launcher (750L): 11 sentence-case fixes (6 quickActions + 2 dealStages + 6 group headings + 3 founder commands); "Thinking..." → "Thinking…"; "Leads/Deals - Quick Actions" → "Leads/Deals — quick actions" (em-dash); dialog wrapper motion.div gains role=dialog + aria-modal + aria-label landmark (was untyped div); backdrop aria-hidden=true; AI Submit button type=button + aria-busy + aria-label flip + focus-visible ring; AI response action button type=button + aria-label + focus-visible; Thinking… role=status + aria-busy + aria-label; AI response panel role=status; 16 inline lucide icons aria-hidden across CommandItems. (commit `53dda87`)
**CreditPurchaseModal radio aria-label + MobileCardList ul/li (384):** ✅ complete — 2-file pair: CreditPurchaseModal — each radio Card gains comprehensive aria-label "${name} pack: ${credits} credits, $${value} value" combining 3 fragmented `<div>`s; visual price/credits/value/Check fragments aria-hidden; Cancel + Purchase buttons type=button; Purchase aria-busy + aria-label flip combining state+selection. MobileCardList — generic card-list `<div>` map → `<ul/li>` semantic so SR gets list count + position. (commit `6076060`)
**DueDiligencePanel sentence-case + flags ul/li + progress aria (385):** ✅ complete — 834L checklist+AI-dossier panel: 12 sentence-case fixes (Strong Buy/AI Dossier Requested/X% Complete/Run All Lookups/AI Dossier/Generate AI Dossier/Try Again/Title Search/Tax Analysis/Market Comps/Owner Research/Legal Access/Access Issues/Issues Found/Risk Level/Green Flags/Red Flags/Executive Summary); ellipsis canonical; top-level loading role=status + aria-busy + sr-only; 4 buttons type=button + aria-busy + aria-label flips (Run all/Generate/Retry/Try again); checklist Progress + Investability/Risk Progress all gain aria-label/aria-labelledby + aria-valuenow/min/max (were missing); Investability/Risk score rows: parent gains computed aria-label "Investability N%" + visual fragments aria-hidden + tabular-nums; Green Flags/Red Flags `<div>` map → `<section aria-labelledby>` + `<ul/li>` semantic; "X% complete" badge aria-label. (commit `d6ac508`)
**DocumentGenerator keyboard cards + ul/li sections + sentence-case (386):** ✅ complete — 1175L 5-form PDF generator dialog (promissory note, warranty deed, settlement statement, offer letter, property flyer): 26 sentence-case fixes spanning dialog title + tabs + 3 section headings + 5 docType labels + 5 generate-button texts + 14 form labels; 3 section `<div>` → `<section aria-labelledby>` + `<ul/li>` semantic; DocumentTypeCard was clicky-Card with NO keyboard support → role=button + tabIndex=0 + aria-pressed + Enter/Space handler + computed aria-label combining label+description+selection + focus-visible ring; visual icon/label/description/dot fragments aria-hidden; open-dialog trigger type=button + aria-label; all 5 form generate buttons aria-busy + aria-label flip; 10 Loader2/Download icons aria-hidden. (commit `44906f0`)
**CompMapOverlay + CohortAnalytics ul/li + sentence-case + tabular-nums (387):** ✅ complete — 2-file pair: CompMapOverlay — "Comp Map" → sentence-case; MapPin aria-hidden; map legend + price-list `<div>` → `<ul/li>` w/ per-row aria-label combining address+distance+price+ppa; tabular-nums on prices. CohortAnalytics — 9 sentence-case fixes (Cohort Conversion Analysis/Lead Source/Import Month/Import Quarter/Total Leads/Overall Close Rate/Best Segment/Conversion Rates/Cohort Detail/Avg Days); removed unused Clock import; loading role=status + aria-busy + sr-only; error/empty CardContent role=alert/status; summary stats `<div>` map → `<ul/li>` w/ per-Card aria-label; 5 lucide icons aria-hidden; table aria-label; all numeric cells tabular-nums; avg-days aria-label "N days". (commit `4342fda`)
**CancellationDialog + ConfirmDialog + CookieConsentBanner button polish (388):** ✅ complete — 3-file trio: CancellationDialog — usage panel `<div>` → `<section aria-labelledby>` + `<ul/li>` w/ per-row aria-label "${name}: ${used} of ${limit} used"; tabular-nums on numbers; all 4 buttons type=button (was missing); Downgrade aria-label "Downgrade instead of cancelling"; Confirm-cancellation aria-busy + aria-label flip. ConfirmDialog — AlertDialogAction gains aria-busy + aria-label flip "${label}" → "Processing: ${label}". CookieConsentBanner — both buttons type=button + aria-label "Decline cookies"/"Accept all cookies" (verbs alone were ambiguous). (commit `31e1e3e`)
**CohortRetentionDashboard ul/li summary + tables aria-label (389):** ✅ complete — 4-card summary + retention heatmap + revenue/conversion table: 9 sentence-case fixes (Total Signups/Total Revenue/Avg Conversion Rate/Revenue per Lead/Retention Curves/Revenue and Conversion by Cohort/Contact Rate/Conv. Rate/Rev/Lead); removed unused Button import; summary cards motion.div map → motion.ul/li w/ per-Card aria-label combining label+value+subtext; both tables aria-label; retention heatmap cells aria-label "Week N: X% retained"; signups badge aria-label + tabular-nums; 5 lucide icons aria-hidden; all numeric values tabular-nums (8 columns in detail table). (commit `55d7469`)
**ContentGeneration sentence-case + ul/li photos + button polish (390 — milestone):** ✅ complete — 3-component share/generate file (SharePropertySheet/ShareDealSheet/GenerateCampaignCopy): 9 sentence-case fixes (Share Property/Share This Property/Share Deal/Share This Deal/Social Post/Case Study/Download Photos/Generate Copy); "Generating..." → "Generating…"; 13 lucide icons aria-hidden; photos panel `<div>` → `<section aria-labelledby>` + image grid `<ul/li>`; photo count tabular-nums; 5 trigger + 3 Copy buttons gain type=button; 3 Copy buttons aria-label flip ("Copy {format}"/"{format} copied"); 3 readOnly Textareas gain aria-label "${format} listing text"; GenerateCampaignCopy button aria-busy + aria-label flip. (commit `8f08edc`)
**CostIndicator + DataConfidenceBadge keyboard tooltip + ul/li sources (391):** ✅ complete — 2-file pair: CostIndicator — Tooltip-on-Badge gains tabIndex=0 + role=img + aria-label "Cost: $X.XX per use" + focus-visible ring (was keyboard-unreachable); icon + visual cost/label aria-hidden; tabular-nums. DataConfidenceBadge — "Data Confidence" → sentence-case; source rows `<motion.div>` → `<motion.li>` inside `<ul aria-labelledby>` w/ per-source aria-label "${name}: ${OK|Miss}, N milliseconds"; visual fragments aria-hidden; trigger button comprehensive aria-label "Data confidence: N%, ${level}. Open source breakdown."; sources-returned `<p>` aria-label + tabular-nums; "Last fetched" wraps `<time dateTime>` + tabular-nums; hoisted lastFetchedIso to single lookup. (commit `bf53695`)
**CustomFields sentence-case + aria-busy + remove-option (392):** ✅ complete — 654L admin manager + per-record editor: 11 sentence-case fixes (Custom Fields CardTitle×2, All Types, Add Field×2, Edit/Add Custom Field, 5 form labels); "Loading custom fields..." → "…"; remove-option literal "x" → typographic "×" w/ aria-hidden + parent button aria-label "Remove option: ${option}" + focus-visible ring (was unlabelled); Add-field trigger type=button + aria-label; submit button aria-busy + 4-way aria-label flip combining edit-mode + loading-state; loading/empty CardContent role=status + aria-busy + aria-label; per-row dropdown trigger aria-label "Actions for ${field.fieldLabel}" (was generic); 5 lucide icons aria-hidden; "(N options)" aria-label + tabular-nums. (commit `b6f2e21`)
**AttributionAnalytics radiogroup + ul/li + table aria-label + sentence-case (393):** ✅ complete — campaign/channel ROI dashboard w/ summary + 2 charts + ROI table: 7 sentence-case fixes (Total Conversions/Revenue Attributed/Best Campaign/Best Channel/Conversions by Channel/Conversion by Touch Number/Avg Days); removed unused Loader2 import; date-range Buttons (30d/60d/90d/1y) → role=radiogroup w/ each Button role=radio + aria-checked + type=button + aria-label "Show last ${range}" (was 4 disconnected toggles); summary cards `<div>` map → `<ul/li>` w/ per-Card aria-label combining label+value; loading role=status + aria-busy + sr-only; error role=alert; empty role=status; 5 lucide icons aria-hidden; campaign ROI table aria-label; all numeric cells tabular-nums; avg-days cell aria-label "${N} days". (commit `dbc657b`)
**CashFlowWaterfall + ClosingCostsCard chart-aria + ul/li (394):** ✅ complete — 2-file pair: CashFlowWaterfall — 5 sentence-case fixes (header×2 + 3 bar names Gross Due/Late Fees/Net Cash Flow); empty `<p>` role=status; chart wrapped in `<div role="img">` w/ computed aria-label "Cash flow waterfall, last 3 months: ${m}: ${amount} net, ..." (was opaque chart); DollarSign aria-hidden. ClosingCostsCard — "Estimated Closing Costs" → sentence-case; loading role=status + aria-busy + sr-only; cost breakdown `<div>` map → `<ul/li>` w/ per-item aria-label "${item}: ${amount}, source: ${source}" combining 3 fragmented spans + tooltip-source; visual fragments aria-hidden; Total badge aria-label + tabular-nums; DollarSign aria-hidden. (commit `8979f79`)
